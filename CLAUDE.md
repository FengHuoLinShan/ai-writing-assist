...
1. **探索任务交给 Explore 子代理**：需要跨多个文件/目录找代码、弄清结构或命名约定时，
   用 Explore agent 只读搜索，模型用 sonnet（本环境映射 deepseek-v4-flash）、effort=max；
   主会话只接收结论，不亲自逐文件扫描。
2. **Plan 之后，执行交给 sonnet**：Plan 产出实现计划后，先把执行阶段按计划划分成合适粒度的
   任务包（相互独立的任务各自成包），再通过 Agent 工具交给 `model: "sonnet"`
   （deepseek-v4-flash）的子代理执行，effort=max。主会话可以自行编写部分代码，划分不强制；
   凡交给子代理执行完成的任务，主会话必须 review 其产出后再验收集成。
...
当前业务模块共 10 个：`account`、`project`、`imports`、`world`、`memory`、`outline`、
`evidence`、`story`、`writing`、`interaction`。RAG 索引与 Context 编译/确认归 `evidence`；
