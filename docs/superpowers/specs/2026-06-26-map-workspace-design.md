...
下一阶段优先实现“空间连续性提示强化 + 写作页到地图页的新标签查看链路”：

- 写作页 Scene 面板清晰展示三类空间连续性提示：缺少主地点、缺少地图上下文、人物跨地图。
- “打开地图”默认在新标签页打开，并携带 `scene_id` / `focus_entity_id`。
- 地图页从写作页打开时默认查看模式，按 Scene → 主地点 → 当前人物/事件 → 势力范围高亮上下文。
- 待确认对象作为默认关闭的独立图层进入地图，必须有明确视觉区分和开关。
- 不做 AI 位置建议，不读正文推断位置，不在写作页内联编辑地图事实。

P0 验收以写作页用户旅程为主，而不是以地图编辑功能清单为主。验收从“作者正在写一个 Scene”开始：写作页显示地图摘要和空间连续性提示；点击“打开地图”后新标签页进入正确地图或明确回退目标；地图页默认查看模式并高亮当前上下文；用户需要时可以手动打开待确认对象图层。地图编辑能力只作为修正路径，不作为本阶段验收主线。
...
从写作页或空间连续性提示打开地图时，默认进入查看模式，并高亮当前 Scene、人物或地点上下文。用户需要修改地点绑定、人物标记或势力范围时，再显式切换到编辑模式。默认不直接进入编辑状态，避免误触和过早暴露复杂编辑控件。

查看模式的高亮优先级为：Scene → 主地点 → 当前人物/事件 → 势力范围。存在 `scene_id` 时，优先高亮当前 Scene 的相关标记和主地点；没有 `scene_id` 但存在 `focus_entity_id` 时，才聚焦单个地点、人物或势力。势力范围作为空间背景信息，不应默认压过当前 Scene 的发生地点和在场对象。

待确认对象可以在第一版进入地图，但必须作为独立图层呈现，且默认关闭。第一版待确认图层只显示已经有结构化地图坐标的候选地图事实，例如 candidate 状态的人物/事件/地点标记或绑定；不把只有正文提及但没有坐标的对象投射到地图上。待确认对象不得与正史对象使用同一视觉语义；需要通过明确的图层开关、弱化样式或待确认标识区分，避免作者把未确认事实当成已确认地图事实。空间连续性提示默认不基于待确认对象图层产生，除非后续显式增加“包含待确认对象”的检查视图。
...
空间连续性提示的修正入口仍然使用写作页顶部“打开地图”能力，默认在浏览器新标签页打开对应地图或回退目标。写作页第一版不做地点绑定、人物标记或势力范围的内联编辑，避免把写作页变成第二个地图编辑器，也避免用户在同一页面反复切换上下文。
