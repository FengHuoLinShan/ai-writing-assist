...
异步 AI 生成/审查/返修任务在领取时冻结运行额度，恢复沿用同一 run；该内部回执只存在任务
私有元数据，不改变 draft、candidate、冲突检查或发布 API 的 wire。
...
ADR-0023 的正文复核是直接只读能力，复用 Writing 回执并共享父预算；原 AI 资料确认与
精确来源校验继续有效。修订/新章经具体预览确认后保存新工作稿，保留来源与恢复基线；
人工来源不会被升级为“已检查人物知识”。已发布与历史版本不被后台建议覆盖。
...
实验入口将三专项调查作为不可信假设传给原语义审稿；正式 finding 仍由 Writing 产生。
