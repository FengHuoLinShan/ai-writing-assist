...
1. **创建草稿**（`POST /drafts`）→ 新版本 + 自动入队 `publish_chapter` 任务（RAG 索引）
2. **更新草稿**（`PUT /drafts/{id}`）→ 原地更新最新版本内容，无副作用（暂存模式）

## Facade

```python
async def create_draft_only(db, novel_id, chapter_index, title, content) -> WritingDraftContract
async def create_draft(db, data) -> WritingDraftResponse     # 创建草稿 + 触发 RAG 索引
...
```
POST   /api/writing/drafts                              # 发布草稿（新版本 + RAG 索引）
GET    /api/writing/drafts/{id}                         # 获取草稿
PUT    /api/writing/drafts/{id}                         # 暂存草稿（原地更新，无副作用）
...
重复保存同一章节时，UniqueConstraint（novel_id + chapter_index + version_number）触发完整性错误。前端收到 409 后提示用户刷新。
