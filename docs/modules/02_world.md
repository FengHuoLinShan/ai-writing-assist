...
目标范围／因果运转两路隔离审查、逐项独立核验、至多一次 confirmed 定向返修、返修后知识复审
（至多一次知识修正并复审）和不再返修的终审。`insufficient` 保持待补证据，`tradeoff` 保持待作者决定；任一 schema、问题 ID、
缺项或超时错误失败关闭。稳定阶段响应与完整回执保存在任务私有结果中供自动重排恢复，公开任务
wire 只返回任务卡和精简检查结论。

保存可携带 `origin_task_id`。服务端重读同项目已完成 design task，并重验父 checkpoint、confirmation、
来源与最终内容 hash（包含有效 depth；省略时继承父阶段）；原结果保存精简审查引用，作者编辑
内容或保存阶段后降级为 `author_edited_unreviewed`。旧回执未绑定 depth 时只能作未复核保存，旧客户端
为 `unreviewed`。终审 blocked 的原样提案不能保存为已检查成果，编辑后仍可作为未复核阶段成果保存。

终审候选与提案统一由返修后的变化重新生成；字段使用 schema 的公开别名，避免依赖边出现两套
名称。`world_design_review_state.v3` 隔离旧输入语义的恢复点，并保留最终知识修正结果，恢复时不重复修正。World 模块
README 记录省略字段继承、完整审查参考与原稿返修契约。
...
资料库查询在分页前优先姓名/别名匹配。人物关联图以当前对象为中心，并显示实际关系类型和可回读依据；历史可展开只读快照。待决定详情将选择写入浏览器历史，返回保留分类、搜索和页码。缺少整体世界设计属于校验前提不足，提供世界核心共创准备入口，不自动创建或改变发布门禁。
...
世界生成中心pro在同一冻结Flash模型上统一max思考与至少65,536输出余量，覆盖生成、知识审查、任务卡、反例复核和返修；fast保留普通配置与必需知识审查。两档继续遵守确认资料、来源重验与1800秒领域护栏。

## 精确身份与地图在场

`find_exact_identity_candidates` 保留同名/有效别名的竞争候选，并限定 novel_id、实体类型及有效
状态；建议影子不参与复用。地图通过 Story facade 读取指定 Scene 的人物位置与出处，详见
[地图场景契约](15_map.md#场景人物位置)，不以档案常驻地补充历史在场。


Evolution 世界候选的 `evolution_ref` 为不可通过普通元数据编辑清除的来源绑定。
采用、合并、转别名、候选关系吸收及采用包必须在无模型调用的短事务中回读该回执与
最新正文/Scene；Project NOWAIT 排他锁覆盖验源至提交，冲突返回可重试错误。
已经采用的作者资产保留历史，不因源稿修改自动删除。生产理解身份查询只使用字面
精确名称，无叙事截止证明的别名继续待裁定。
