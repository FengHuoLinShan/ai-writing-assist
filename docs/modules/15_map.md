...
| POST | `/{map_id}/observations?novel_id={}` | 创建地图 observation；201 表示事务已提交，可立即读取或确认 |
...
创建 observation 的 201 只在事务提交后返回；后续确认、列表和地图状态请求可以立即通过
独立数据库连接读取该 observation。提交失败返回脱敏 500，不返回不可继续操作的伪成功 ID。
