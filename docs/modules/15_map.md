...
- 持久化共创会话（ADR-0021，`world_cocreation_*`）只是讨论与成果引用记录，不进入地图册的
  空间指纹、生成上下文或采用状态机；地图继续只消费经作者确认的资料。
...
结构编辑支持区域、城市、街区、街道和作者显式创建的室内示意。室内沿用同一来源、图元、
图片和版本合同；示意布局不证明原作未给出的精确尺寸或平面位置。
纯空间节点调整目录与顺序无需存在生图任务；未绑定世界地点的地图可修改标题，绑定地点仍禁止
从地图界面改名。已有空间版本的节点仅允许区域／城市／街区／街道／室内层级，避免目录调整后无法继续编辑。
...
已采用图片在“查看图片参考”下可显式开启双图对照，默认关闭。左右选择保持不同图片，切换节点或
图片移出时重新校验选择；对照仅改变展示，沿用私有图片读取，窄屏上下排列。选择器使用标题、
已知展示用途与日期；缺少来源或补全元数据时显示“未记录”，不把缺失记录解释为没有视觉补全。
...
- 关系提取首轮即提供完整输出 schema；每个来源分别携带并核验引文，不能用一段引文冒充多个来源。
  空关系数组与缺失 `relations` 区别处理。任务结果与地图读取的 `generation_summary` 保留安全摘要、
  覆盖范围、批次/裁剪/舍弃原因和实际结构化尝试次数；未取得次数时为 null，不推测重试，也不返回
  原始无效模型输出、Prompt 或密钥。诊断日志写入失败不属于确认的正文资料或指纹。
...
- 逐项采用前，`POST /{novel_id}/nodes/{node_id}/revisions/{revision_id}/review-preview` 使用相同
  基准/来源/依赖校验，只读返回实际和扩展的修改键；作者核对后再调用 `review`，后者仍重新校验。
- 背景图片层返回 `calibration_revision_id` 与 `calibration_lookup_status`（found/not_found/truncated）。
  只从同项目、同节点、同空间指纹的已保存版本中，按图片/用途/锚点精确匹配最近可证明的校准版本；
  最多检查 100 项并用下一项判断截断。它不是首次校准事件，也不是图片生成来源；找不到时不猜测。
  地点配图这两个字段为 null，过期底图仍退出叠加，图片读取与读者白名单不因对照放宽。
...
- 编辑撤销回已保存状态后，自动清理仅针对本页已知且未被其他标签页替换的本机快照；首次尚未
  读回的备份、待决定恢复内容及其他账户数据仍保留，不因刷新或卸载静默丢失。
- 待恢复备份未决定前可浏览但不可改图或采用其他版本。放弃使用页内二次确认并重验当前快照；
  另一处已变化时要求重新核对。损坏备份不能恢复，但可明确二次确认放弃；读取失败保持保护。

作者画布旁显示选中内容；地点绑定搜索独立、多图绑定显式逐图 CAS 保存，离开支持保存或已核对的本机草稿。空间图元、Evidence 来源、图片门禁保持。
第四期影响预演（ADR-0022）以同模块只读方式读取 `map_atlas_nodes.location_entity_id` 列示受世界对象变化影响的地图节点；地图域不接收任何写回，标注引用列入预演的未覆盖说明。

## 助手消费

ADR-0023 经 Evidence author-only inspection 读取当前采用的空间版本和失效标记，不返回
私有图片URL，不推造缺失地理。助手可准备空白节点与既有版本审查，仍由 Map 原有基线、
来源、预览和采用流程决定结果；图像/校准/复杂编辑继续保留受控专业入口。


Assistant 地图读取/修改结果保留 node 与 revision，图片状态摘要保留 run 与 page。前端按指定
版本进行比较，不能用当前地图冒充旧结果。地图能力接口按结构、上传、生图区分依赖；私有
存储缺配置不禁用空间编辑，图片提交先检查配置，外部画面说明仍经过原资料确认。

### World 变更影响预演的边界

World 的只读影响预演通过当前地图节点的 `location_entity_id` 枚举关联；它不修改地图状态，也不代表已经验证地图标注或场景脚本。正文按名称与已采用别名字面扫描，达到扫描上限会显示未覆盖范围；该结果不能用于声称完整语义检查或自动重绘地图。

### 世界复核来源

World 影响清单将地图节点绑定到当前空间 revision，来源读取验证 node/revision 与 hash；经 Evidence 显式选入的地图结构可参与有界世界语义复核。该来源仅用于无 Scene 截止的作者视角，检查空间声明和依据，不执行图像视觉几何识别，也不反向修改地图或 World 正式事实。

World 设计精细模式的反例审查仍只核对本轮 typed 世界变化及其冻结来源；即使检查到资源、执行或
长期反馈，也不等于验证地图坐标、路线几何、当前 Atlas revision 或图片内容。审查与 checkpoint 保存
均不触发地图重算、重绘或采用，地图变化仍须走本模块现有来源与 revision 门禁。

### 审阅与导航体验约定

地图默认画布优先，所选内容详情按需打开；窄屏目录收拢为地图选择器，缩进不持续挤压标题。版本比较主动定位差异区并显示历史/基线时间，返回恢复触发位置；位置锁定统一禁用微调，使用服务器版成功后清除已解决冲突提示。
