...
- 看海是前台 heartbeat 驱动的确定性有界 step 循环。显式离开故事页立即撤销 heartbeat，
  浏览器短暂隐藏才使用宽限期；关闭或离开后都不再创建后继。工具仅由 ADR-0023 有界运行时开放；看海循环仍由代码编排
  工作流。
...
长期约定由用户保存到既有回顾JSON，自动摘要只输出原七区，服务端继承约定。首次无回顾可保存，
...
故事v7保留约定接入和既有写作风格；v6正文实验未通过模型/用户评审，未进入保留范围。

## 有界单 Agent（ADR-0023）

新快照启用后每轮先自主查证再由原 attempt 流式持久化；旧快照仍用原 handler。
选中路径、长期约定、固定资料版本、人物截止及离页停止看海保持有效。后台旅程复核只给
有成对精确引文的连续性建议，不回写故事。联网仅用于通用事实，外部内容再次检查后才能进入资料。


RP 新运行 v2 使用显式的旅程 `web_search_enabled` 与冻结 SearXNG 后端。关闭只阻止新外部
