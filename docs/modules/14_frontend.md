...
所有作者主动发起的创作模型操作复用 `shared/aiReferenceModal.js`。打开即自动预览；预览有效
前、选择变更未重编译、有 blocker 或 AI 调整提议尚未处理时，“按这份资料开始”不可用。
首屏按“必须使用 / 系统找到 / 我添加的 / 本次不用”展示 ContextItem，不暴露 ID、JSON、
Prompt 或 token；预算遗漏另行解释。作者可逐项移除/恢复、用现有 Evidence Search 添加资料，
或让项目模型提出增减 patch，只有点击“应用调整”才改变本地选择并重新编译。
提交最终 confirmation 后，范围、要求、搜索、逐项操作和重新整理均冻结到响应结束；响应落地
前再次核对 dirty 状态与已审查指纹，避免用旧选择关闭弹窗并启动任务。取消仍保持可用。

弹窗保留统一 modal seam，不引入前端依赖；错误使用 `role=alert` 并恢复焦点，状态使用单一
`aria-live`，按钮具备 disabled 语义与至少 44px 点击区。小于 640px 使用整屏布局，正文和页脚
均可滚动/操作。故事总览、生成中心、Writing、Story、World、问世界、地图册和 Scene 融合的
公开请求都携带 `context_confirmation_id`；任务内部复核继续复用原 ID，不重复弹窗。
...
`WorkspaceToolCard` 是故事结构、地图与世界模块的共享展示组件。桌面使用唯一
`#sidebar-context-slot`；手机通过命名按钮打开 body 层 `WorkspaceDrawer`，避免滚动容器
裁切关闭入口，并在动作执行前释放抽屉焦点边界。工具卡只消费页面生成的动作/状态，
不持有业务资产或独立任务队列；不新增服务端 API、schema 或授权路径。

故事工具随总览、篇章、剧情线、场景分区及明确选择切换，优先继续编辑、查看运行进度、
核对候选。总览生成和结构分析共享当前页进度区。地图编辑器在原 `state` 事件中提供节点身份
和工具状态，工作台验证节点身份后渲染，经 `runToolbarAction` 回到原操作实现。阅读预览
不从作者数据生成工具名称或数量；保存、采用、草稿恢复和冲突继续使用原保护路径。

`ActionMenu` 的可选浮层模式处理侧栏裁切、可视视口和键盘焦点。菜单动作展开期间固定身份，
执行前复验可用性。智能去重仍沿用原静态按钮挂载与服务，通过 document 委托支持 body 层
工具抽屉；工具卸载、菜单关闭和移动断点切换清理浮层及监听，不创建全局业务状态注册表。
...
- RP source 向导只使用读者语言，不展示 UUID/JSON/task/token/Prompt。四步只挂载当前决定，已完成
  ...
  Escape/焦点返回，390px 下步骤摘要两列换行、44px 操作区且不增加 composer 常驻按钮。作品整理
  与故事生成使用用户账户中连接的 AI 服务；请求经本站后端代理，Key 不进入浏览器或项目。
...
- 资料库使用 `layout=cards|list`（list 为默认，默认值不写回 URL）与 `q/kind/type/state/topic_id/fav/unclassified/sort/skip`；服务端统一列表负责结果集与分页。旧对象库 `mode=normal|hot` 只在兼容组件中保留。Scene 工作台仍使用 `mode=normal|hot`，URL 优先于项目偏好。
...
- 生成中心 world 工作区接入服务端持久化共创会话（ADR-0021）：进入时按“来源 + preset + target”水合最新会话或 `session_id` 深链，消息以服务端终态记录为准并保留本地未完成气泡；发送首条消息时自动建会话，聊天回合绑定来源 confirmation 与动作标签，异步建议任务成功后由 worker 把成果回合写回会话并刷新本地记录，作者决定（收束确认）落为 decision 消息。会话条目按建议现状显示“讨论中／待审阅／已存工作稿／已采用（或作者已否定）”，待审阅可直达审阅；阶段成果保存后在服务端推进 `current_checkpoint_id` 指针，漂移 409 时保留提案、提示作者核对后再次保存。工作区提供“历史会话”弹窗（打开/归档/新建），跨设备清空本地缓存仍可从服务器继续；服务端不可用时静默回退本地 v2 会话，长任务回执与“刷新后不自动重复提交”语义不变。
- 资料页阅读态支持项目内 Wiki 引用：`[[名称]]` 渲染为可点击 chip（复用安全 Markdown 渲染，Vue 节点输出，名称与内容均按纯文本转义，不执行 HTML 或资料中的指令）；点击后按项目内名称精确解析——唯一命中直接打开，重名弹出选择列表（名称、类型、概要）由作者选定，未找到仅提示不导航。引用解析只读，不写回任何对象或建议。
...
- 世界书创设建议弹窗只编辑标题、类别和页面概览；结构化分区与资产关联从原建议原样写入工作稿，再由现有页面编辑器承接作者修改。前端不提供 JSON 输入，也不改变 `world_bible_page_draft.v1` payload/wire。
- 世界书编辑器把“保存并发布”作为始终可见的主操作；点击后先保存工作稿并显示发布前影响
  ...
  只有“保存工作稿”不会改变正式页。
- 世界书工作稿编辑支持可靠保存：输入先写本机备份（localStorage 按项目 + 页面/工作稿隔离），
  停止输入 1 秒后自动保存服务器工作稿并携带 `expected_updated_at` 基线；同时最多一个保存请求，
  新输入在前次完成后继续保存，晚到响应不回写基线也不重置输入，排队中的下一次保存复用同一份
  工作稿并携带服务器最新基线（首次自动保存创建的工作稿即时登记，不会重复创建），自动保存不
  发布页面。刷新或重新打开时，本机备份只在进入编辑态后提示恢复；阅读态不提示、也不因未选择
  而清掉备份，保存成功后按页面/工作稿两个作用域一并清理。手动保存在
  自动保存进行中会先等其完成再携带最新基线。基线冲突（另一标签页已更新）时暂停自动保存并
  保留输入，同时弹出对照弹窗逐字段展示“我的输入（未保存）”与服务器版本（标题、页面概览、
  分区数、更新时间），由作者显式选择“保留我的修改”（自动保存保持暂停，继续编辑后手动保存）
  或“采用服务器版本”（重置基线、清掉本机备份并把服务器内容写回表单）；服务器版本读取失败时
  保留对照框架与刷新指引，不静默丢弃任一侧。编辑器标题旁以命令式状态文本展示
  “待自动保存/正在保存/已暂停”。
  编辑器输入为 DOM 驱动的非受控表单，任何输入期间的响应式重渲染都会重置输入框，保存状态
  因此必须命令式写入 DOM。`Ctrl/Cmd+S` 在输入框内也可触发保存（经 route host 的
  `shell:save-request` seam）；`Ctrl/Cmd+K` 在 world 页内打开资料快速搜索（统一资料列表、↑↓
  选择、Enter 打开并记录最近访问），其他页面回退命令栏。对象与人物档案编辑同样携带基线；
  对象详情的名称、概要、公开信息与作者秘密支持就地编辑（显式保存/取消、不做失焦保存），
  409 时在表单旁展示服务器版本的字段值，由作者选择采用服务器版本或保留自己的输入。
- 世界书资料页默认阅读态：从资料库列表、卡片、深链或快速打开进入已发布页时先渲染正文
  （页面概览 + 分区 Markdown），配标题目录（点击定位）、分区折叠和引用资料预览（引用以
  名称 chip 只读展示，失效引用标注“不可用引用”）；点“编辑”进入现有编辑器，“完成编辑”
  返回阅读态并先冲刷在途自动保存。工作稿深链与新建页面流程仍直接进入编辑，发布成功后落在
  该页阅读态。阅读器 960px 以下目录内联为正文上方，760px 以下保持至少 44px 触控目标；
  阅读态不改变实体打开、资产关联与发布契约。
...
- 世界书新建页面不把兼容用 `custom` 当作可创建分类；“新建分类”在同一弹窗进入两步
  流程，保留已填页面标题和模板，分类创建成功后返回并自动选中。创建和编辑只展示名称、
  说明与可选外观，稳定键、排序数字和 raw status 不暴露给作者；预设分类已存在时复用，
  已归档时显式恢复。顶部“更多工具”是不参与工具栏排版的覆盖式下拉菜单，支持 Escape、
  点击外部关闭和焦点恢复。
...
- 别名批量采用只提交 `worldSession` 中与当前执行指纹一致的已准备草稿，归属对象、trim 后别名、名称用途或具体称呼任一缺失都在请求前阻断；批量忽略不要求草稿。批量请求绑定确认执行时的作品、页面与子页，晚到的成功或失败响应在上下文改变后静默丢弃，不修改新页面会话、提示或刷新。单条采用成功时先提交下一条 `review_item` 与当前焦点，再调用会卸载重挂的 `router.refresh()`；新组件按 URL 恢复下一条，末项清除定位。
...
- 首次主操作为“新建地图”，区域/城市示意无需模型或图片连接；空间图和图片共用节点目录。
- 无图片的空间节点可调整标题、父级和顺序；绑定世界地点的地图不在此改名。地图支持查找定位、
  专注浏览、键盘选择与地点配图；专注浏览不允许拖拽或方向键改图。
- 空间候选按图元、关系、图片配置和标注绑定显示差异，可对照已保存地图；查看不覆盖当前编辑，
  采用仍须通过原 confirmation 和版本基准门禁。加载会显示当前来源失效提示，历史保持不可变。
...
Vue `FocusedEvidencePanel` 用于地图与写作副驾驶，展示根/直接关联对象、原文出处、
身份歧义、已扫描/待查范围及继续入口；选中项只传 SourceRangeRef/TargetRef，生成前
复用 AI 参考资料确认。`TargetedCompletionPanel` 位于对象详情和导入结果，明确授权
新增/填空，展示待审数量与安全撤销，partial 撤销不显示全部完成。
停止请求失败不使原轮询失效；任务查询返回 404 后解除忙碌状态并提示重新开始。

面板复用现有 bridge、任务轮询、账号作用域的恢复记录与主题语义 token。新能力按需展开，
不替代写作主操作；切 Scene/节点/项目的晚到响应失效。动态文字使用 Vue 转义，
390px、键盘、失败恢复和不覆盖作者编辑均纳入浏览器验收。
### 地图与写作持续工作流
