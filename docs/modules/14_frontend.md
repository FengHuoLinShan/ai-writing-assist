...
写作候选与 Story P20 预览在原成果卡内共用一个懒加载 `<details>`，组合
已持久化 Confirmation、确切 task operation、知识复核、结果引用与失效事实。
组件切换成果或卸载后忽略旧响应；task 已清理时仍展示来源和成果，单次读取失败只提供
就地重试，不阻断采用、拒绝或编辑。链接复用现有成果定位器，不显示 raw ID。
...
| `vue/views/interaction/DemoRpView.vue` | `demo-rp` 路由；仅在公开演示配置开启时公开加载资料并恢复已有匿名 session 的最近旅程。无 session 不自动创建身份，用户必须勾选当前认证配置的用户协议和隐私政策并点击开始后才创建。匿名 RP 会话使用独立 Cookie，不替换已登录作者会话。开场、流式、分支、重抽、纠正、回顾和行动选项复用既有 interaction wire；不显示看海、主动后台续写或网页搜索。临时 DeepSeek Key 只存当前页面的 JavaScript 内存，仅随 direct SSE header 发送，刷新、关闭、匿名 session 或账户边界失效时清除。 |
...
- 世界书资料页默认阅读态：从资料库列表、卡片、深链或快速打开进入已发布页时先渲染正文
  ...
  该页阅读态。阅读器 1100px 以下目录内联为正文上方，760px 以下保持至少 44px 触控目标；
...
- 世界书内的“世界健康”折叠面板显示待作者决定、待补证据、失效数和复核进度，支持当前
  ...
  （已修正/已知悉/稍后再定，绑定回执可追溯）、分片覆盖进度条、失败/预算中断后的
  “继续校验”续接（已检查分片不重复执行）、失效原因的作者语言提示（政策/内容/依赖/目标
  已变化），以及“编辑校验政策”结构化编辑器（原则与禁区规则、按知识层分组的必问项、
  高级预算）——政策保存为《世界书校验策略》页工作稿，仍走发布校验才生效。页面与对象
  详情提供“影响预演”弹窗：列出世界页/对象关系/人物资料/故事线/正文（章节字面）/
  地图的可证明依赖与显式未覆盖说明；引用编辑每条引用可选 参考/依赖/派生/冲突，
  关联图区分资料关联与已声明依赖并可只看依赖。主界面不展示
  raw ID、JSON、Prompt、token 或 hash。旧项目默认只自愿校验；作者二次确认“启用发布前校验”
  后才强制发布/采用门禁。warning 可写理由整体签收或逐项处置；stale/block 明确要求重跑或修正。
...
- 版本历史/恢复
- 正文选区批注：保留原生 textarea，以只读镜像按码点锚点常驻高亮；右侧卡片定位、失效提示和批量执行。长任务保存 task ID，离开页面后可回看结果；只打开候选供比较与显式采用，相关设定提案进入独立助手确认视图
...
影响、依赖和逐组回执；确认前校验未保存正文。讨论与提醒采用独立面板视图，1100px 及以下改为焦点受控的覆盖面板，不与侧栏同时挤压工作区。
...
共创浏览器缓存按项目、创作绑定和服务器会话分别保存；切换会话不混入旧决定、未发送输入或任务收据。恢复及生成成果刷新使用最近终态消息。服务端历史持久化不等于已经提供完整消息检索界面，当前历史列表仍有展示上限。

世界书本地备份记录编辑基线；基线不一致或旧备份无基线时，恢复到表单后暂停自动保存并要求核对。选择保留本地修改后，作者可使用刚核对的服务器基线手动保存；采用服务器版本包含清空分区。手动保存的晚到响应不能清掉新输入的备份或带着新输入进入发布。人物档案保存期间的新输入继续标为未保存；切换对象后不接纳旧对象的响应。切换账号清理世界书备份。

世界健康区分加载、失败、筛选无匹配与真正没有问题；失败可重试，空筛选仍能清除，晚到列表不能覆盖新筛选。稍后再定项可继续裁定；语义续接复用回执的原参考确认，不重新选择范围。政策编辑固定打开编辑器时的工作稿基线；数值容差和字段比较使用带标签的原生字段，手机按单列排列。

### 世界工作区的数据加载与版本边界

资料库首屏由 library overview/分页驱动，不预载所有页面与工作稿；页面深链只取目标与对应工作稿，引用查找使用服务器目录。辅助模板、规则、简介和健康状态在入口打开后读取，并区分未加载、失败与实际空态。

WorldDesignPanel 编辑本轮 typed 变化，并将知识来源审查与设计质量检查分开展示。精细模式只显示
“本轮理解”和作者可行动的检查结论：已解决、待补证据、待作者决定、终审状态与检查面向，不显示
内部角色、问题 ID、Prompt 或推理。passed 正常保存；有开放项可保存候选 checkpoint；blocked 原样
不可保存为已检查成果，作者编辑后可保存未复核阶段成果。CocreationHistory 提供历史分页/定位与明确
参考选择；聊天/推演的 operation receipt 按 project/session 归属恢复，晚到结果不能覆盖别处输入。
历史模型另建会话继续；未保存推演可从历史查看，当前编辑保留。影响来源精确回读、可返回清单，
复核来源在健康面板就地打开。


### 审阅与导航体验约定

写作结构警报直达当前场景字段；章节目录支持章号/标题查询和当前章定位。已保存且与正式正文一致时不重复提供发布操作。检查分清未运行、运行中、失败、部分完成和完成无问题；同版本及内容hash可回看上次检查，也可明确重新运行规则检查。篇章行编辑支持取消/Escape，未修改不算未保存；任务添加直接打开表单。
...
两处共用账户/项目级 forecastStore 与一个十五秒轮询，激活中的宿主持有当前任务焦点。
授权范围、证据快照、任务意图和显示焦点分别比对；已保存选区、光标段落和排除项进入请求。
逐方向拒绝绑定稳定身份，旧保存版本和晚到请求不能覆盖新状态。润色/修改方向将原选区
与意图交给讨论入口，未发送输入冲突时保留草稿。
...
CreativeExperiments 的保留理解授权默认关闭；CognitionPanel 按需读独立历史，修正/撤回沿
原 API 的 head/revision 防冲突，失联重试复用 operation。未保存输入同时受路由和助手标签
切换保护，项目切换拒绝旧响应。作者修正与来源新鲜度分开呈现。
MapStructureEditor 在已保存作者视图内加载 MapScenePanel，场景选择由服务器聚合在场、
最后出现、未知与来源；读者/候选/未保存地图不显示作者面板。选择或版本变化先移除旧结果，
晚到响应与地图版本不匹配均拒绝；不为未知位置生成坐标或推测行程。

项目偏好“高级导入”和写作页“理解与整理正文”共用 ReadingFlow。
项目偏好展开才读取状态；写作弹窗打开即查明运行归属，已切换项目优先显示理解操作，
旧流程项目保留原完整/分阶段整理和质量档位。状态未能读取时不展示旧启动按钮。
整理历史仍能查阅，但服务端 `can_continue` 与当前归属共同控制旧继续/重算，
查漏详情保留只读模式，不因历史恢复旗标残留而显示可启动旧 owner 的按钮。
ReadingFlow 提供显式启用的逐场景理解试用，
仅运行中轮询，关闭/离开停止；预览范围不调用模型，确认后执行。支持连续推进、追加、
安全恢复和明确暂停；费用未知不提供直接重试。网络失联保留原操作身份，明确拒绝则
释放旧确认以重新准备；项目切换丢弃晚到响应及尚未完成的旧确认。没有场景的正文末尾
先准备边界，预览明确总场景数尚未知；准备、纠偏和理解共用本次调用额度，额度耗尽
需显式追加，边界待决定则引导在故事大纲确认后恢复。来源变化可预览重算后缀，
引用失败可明确选择从第几场重新核对；预览展示继承场数、实际起点和扩大范围，
确认新调用额度后才启动，旧费用与结果保留。可按人物/对象或已有观察检索，显示原文并
选择核对目标；搜索有分页，无匹配时允许改用场景范围。预览说明目标只负责定位，实际
会重读该场及后续依赖场景。不将此入口表示为全部导入替代。
