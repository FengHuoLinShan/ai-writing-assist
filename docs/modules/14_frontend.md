...
地图编辑首屏先显示定位控件和画布；阅读预览及查证资料放在画布之后，保持键盘顺序与视觉顺序一致。
...
- 自动整理确认复用共享流水线授权文案；可靠结果可自动采用，需判断的内容待审。进度步骤使用中文，内部批次编号不在主进度卡展示。
- 导入附带的补全面板读取子阶段状态，前序整理中显示等待，补全结束后不随后续结构整理误报运行；整次导入的停止和继续仅由父任务入口处理。
- 查找页区分服务连接与资料健康。当前降级覆盖历史健康标签，失败片段重试位于提示旁；重建结束后回读状态，更新剩余失败数量和章节覆盖。所有操作继续复用现有 API、项目隔离与任务恢复契约。

- 世界对象待处理分组使用同一中文类型目录与来源序号；逐字段原文合并去重后按需展开，内部批次和场景 ID 收在诊断详情，不占默认队列。此投影不改变证据、对象身份或裁决请求。

- 地图专注切换使用保留实例的隐藏方式，避免卸载查证面板而丢失本机输入和完成结果；切换地图节点与阅读权限边界仍按既有隔离规则处理。

专项查读遇到提名阶段暂停时保留已读取证据和续查入口；前端不把部分覆盖描述为完整查读。

- 查证面板将跨根对象/关联层的覆盖计数标为章节查读次数，避免60章作品的两层复查显示成120章。

- 智能去重扫描完成后保留「查看去重建议」入口，不抢占其他编辑/审阅页面；只有仍打开的扫描进度窗可原地进入结果。对比表默认显示中文状态、章节和场景序号，完整业务数据收在诊断详情，别名用名称列表呈现。

- 去重裁决提交仅包含资料不重叠的就绪组；共享成员的组保留决策，待前一批完成后继续提交，并重新接受服务端指纹校验。

- 篇章和剧情线默认标记只保留来源及待检查状态，不展示内部处理阶段枚举。

作者整理入口、异步离开守卫、连续审阅与地图就地编辑的当前契约见 frontend-console/README.md；本轮没有引入新的路由宿主、框架或依赖。
