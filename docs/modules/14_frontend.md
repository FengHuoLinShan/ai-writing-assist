...
WorldDesignPanel 编辑本轮 typed 变化，CocreationHistory 提供历史分页/定位与明确参考选择；聊天/推演的 operation receipt 按 project/session 归属恢复，晚到结果不能覆盖别处输入。历史模型另建会话继续；未保存推演可从历史查看，当前编辑保留。影响来源精确回读、可返回清单，复核来源在健康面板就地打开。
