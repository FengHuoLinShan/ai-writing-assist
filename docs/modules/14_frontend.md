...
- 别名批量采用只提交 `worldSession` 中与当前执行指纹一致的已准备草稿，归属对象、trim 后别名、名称用途或具体称呼任一缺失都在请求前阻断；批量忽略不要求草稿。单条采用成功时先提交下一条 `review_item` 与当前焦点，再调用会卸载重挂的 `router.refresh()`；新组件按 URL 恢复下一条，末项清除定位。
