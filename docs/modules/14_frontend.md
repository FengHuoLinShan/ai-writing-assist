...
- 世界书工作稿编辑支持可靠保存：输入先写本机备份（localStorage 按项目 + 页面/工作稿隔离），
  ...
  自动保存进行中会先等其完成再携带最新基线。基线冲突（另一标签页已更新）时暂停自动保存、
  保留输入并提示刷新对照；编辑器标题旁以命令式状态文本展示“待自动保存/正在保存/已暂停”。
  ...
  选择、Enter 打开并记录最近访问），其他页面回退命令栏。对象与人物档案编辑同样携带基线并把
  409 翻译为“已在别处更新，输入已保留”的作者提示。
