...
| `--nc-dim` | `#6B6964` | `#8A8680` | `#626966` | 辅助文字（与页面底和卡片底均 ≥4.5:1） |
...
- ≤760px 顶栏隐藏模块面包屑与品牌文字，保留品牌标记、截断的当前作品名、服务连接状态、主题和
  账户入口；≤360px 写作页只在 MobileQuickNote 头部同时呈现本章与今日累计后隐藏顶栏字数胶囊。
- 页面级横向溢出零容忍（390px）。
