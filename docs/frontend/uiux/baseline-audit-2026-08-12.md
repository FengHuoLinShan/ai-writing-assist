...
- 40 张视觉基线快照最后一次更新为 `724f802ca`（2026-08-04，main 上的提交）；
...
- 在最新 `origin/main` 上移植本报告对应的 10 个后续提交，修复页头渐进菜单被
  `overflow: hidden` 裁切的问题；写作视图执行菜单动作后立即关闭。
- 390px 移动速记改为占用工作区剩余高度，不再重复用视口公式和固定底栏高度计算；发布、保存、
  完整编辑器三项操作保持在五项移动导航上方。
- 校正基线总数为 40，并恢复移植冲突中遗漏的剧情线、世界书三主题基线；移动速记基线按新首屏重建。
- 验证：生产构建通过；Vitest 163 文件 / 1800 用例全绿；作者端 smoke 47/47；视觉回归
  14/14；`make docs-check BASE_REF=origin/main` 通过。
