...
12. **【低】「导入已有作品」按钮行为分叉**：无项目时点击直接弹文件选择器
    （ProjectView.vue:76-82 toggleImportSection），有项目时开合抽屉——同一按钮两种行为无提示
    （执行时核实产品意图，或在按钮文案/hint 上区分）。
...
- 表单映射 §5.2：label（`--text-sm` secondary）→ 控件 → helper（accept 白名单说明 :121
  保留为 helper 文案）。
- 按钮层级：抽屉打开时「上传并导入」（:131-136，btn-primary）与 hero「新建空白作品」同屏，
  违反 §5.1「每屏至多一个 primary」——裁定：上传按钮降级为默认 `.btn`（执行时核实 e2e 对
  class 的断言）。「导入为新项目」保持 ghost（:137）。
...
- 按钮语义修正：「批量恢复」不应是 `.btn-primary`（:46）——恢复是常规操作，改默认 `.btn`；
  「永久删除」系保持 `.btn-danger`（:47、:69）；批量/单项永久删除的二次确认（:146、:176）
  保留，确认主按钮文案写动作本身（§5.6）。
...
| 状态 | 现状 | 缺口 | 目标形态 |
|---|---|---|---|
| 首次进入（无项目） | 引导空态「开始你的第一部小说」（:194-204） | 无 CTA 按钮，只文案引用上方入口 | 补 `.empty-state-cta`「新建空白作品」主按钮（§5.9）；hero 主操作保持可用 |
...
- [ ] 「管理作品」toggle 在有已选项时显示计数；批量删除二次确认保持。
- [ ] 首次空态含 `.empty-state-cta`；网格加载有骨架。
- [ ] 断点归并到 760/1100 体系（保留的局部断点有注释）；390px 无横向溢出。
- [ ] §7 全部契约存在且可被对应 selector 选中；selectors.js 同步。
