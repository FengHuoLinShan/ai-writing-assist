...
- `OutlineHeader` 保留四个子标签、标题、计数和作品名；场景的普通/热点切换仍在页内。
  全页动作集中到共享侧栏「故事工具」，手机使用命名入口和可关闭抽屉。
- 故事总览：手工编辑、AI 总览、进入篇章规划、结构分析；篇章：新建、AI 规划、
  正文整理、结构分析；剧情线：新建、AI 创作、正文整理、伏笔与揭示；场景：
  AI 细纲、正文整理、待整理场景、场景建议。智能去重与历史等低频工具按当前页面收入更多菜单。
- 主操作优先返回未完成编辑、查看进度、核对建议或处理当前选择。明确选择篇章/剧情线后，
  AI 入口复用原「修订所选」能力；场景复用原上下文待办判断，批量确认仍在原批量操作条。
- 人物卡、推演、剧本分区使用本场工具；未选场景时明确禁用相关动作。表单保存和候选采用
  保留原位，侧栏只把用户带回对应编辑或核对区。缺少已采用总览时沿用原 AI 规划前置门禁。
- 本节工具位置取代下文历史修复记录中的旧标题操作组；原 `data-action` 业务动作、
  参考资料确认、草稿与离开保护、任务恢复及 API 契约继续保留。

### 3.2 story 故事总览

单列纵向流：任务进度卡（仅有任务时）→ 主操作区 → AI 预览编辑器（创作核心 4 文本域 +
剧情线/故事推进/待决定问题三个可排序列表）→ 当前版本连续阅读文档 → 默认收起的过往版本。
无版本时保留唯一首次进入说明，工具卡突出「AI 生成总览」；有版本时说明切换为「调整整体方向」，
工具卡以手工编辑为主操作、AI 新方案为次操作。当前文档按版本与标题、故事核心、总览正文、主要剧情线、故事推进、待决定
