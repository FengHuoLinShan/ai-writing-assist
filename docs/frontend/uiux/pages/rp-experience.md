...
10. **已解决：导出反馈闭环**。完整记录和故事正文下载触发后分别显示明确成功 toast；失败仍说明
    旅程内容未受影响。
11. **已解决：关键请求有按钮级 loading**。开场创建、发送、停止、继续与重新生成均在原按钮
    显示 spinner、动作文字或动态可访问名称，保持按钮尺寸并禁用重复提交；reduced-motion 下
    spinner 降为静态圆环。归档与永久删除的忙碌反馈由确认层承载。
12. **已解决（2026-09-02）—历史段落无法原位重新生成**：当前发展上的每条
    story 都提供「重新生成」；历史段落先用现有 RP 确认层说明会从此建立新分支，
    之后内容仍可在分支树找回。最新段落保留一键重新生成。
...
- 发送/停止按钮使用 spinner 替换图标并更新可访问名称，宽度不抖动；pending 期间禁用。
...
- 发送钮使用 spinner + disabled +「正在开始旅程」可访问名称，宽度保持不变。
...
| 开场创建中 | 原发送钮显示 spinner，`aria-busy=true`，名称为「正在开始旅程」 | 无 | 保持；失败继续保留开场草稿 |
...
| 生成失败/取消 | `role=alert` 按 action 分派；重试显示 spinner 与「正在重新生成」 | 无 | 保持按钮级 busy 与重复提交锁 |
