...
### 3.2 journeys（旅程列表 + 开场创建，JourneyListView.vue，505 行）

- Primary：开场创建卡（textarea「旅程开场」+「开始旅程」），新用户的第一动作。
- Secondary：旅程目录（`role=tablist` 进行中/已归档 + 搜索 + 卡片列表 + 加载更多）。
- Tertiary：吸顶 header（返回箭头、「账户设置」）、模式开关行、字数/快捷键/数据告知小字。
- 第一视觉焦点：开场 textarea（空态时）或第一张进行中旅程卡（有进行中旅程时）。
- 阅读路径：标题 → 开场卡 → 目录；「账户设置」是次级出口，不得在视觉上与主操作竞争。
...
- textarea + 圆形发送钮 + 模式开关行 + 看海确认 popover + 字数/错误/快捷键/数据告知
  （JourneyListView.vue:326-407）结构保留。
