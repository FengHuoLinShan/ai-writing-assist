...
- **核心循环定位**：审核队列是本页的**第一优先级区域**。AI 只产出建议，作者决定正史；
  「需要决定」计数是全产品统一的待处理信号，视觉必须常显、可读、可一键进入。
...
16. **已裁定：不抽象诊断 `<details>`**。当前是一个带筛选状态的「诊断筛选」和
    三个只显示本队列加载原因的原生 `<details>`；状态、内容和所属队列不同。
    现在抽组件只会增加 pass-through props，不降低使用负担，因此保留原生元素。
17. **已解决（2026-09-02）—单页总数不可见**：`WorldPager` 在有结果时始终显示
    「共 N 条」，只在超过一页时显示上一页/下一页；空列表仍由各页空态解释。
...
- **资料库**（bible）：用统一 page/entity Card 投影组合资料页、工作稿和已采用对象；旧
  objects/aliases 只作深链与批量工具入口，高亮归并到资料库。
- **关系**（relations）：管理已采用关系。
- **需要决定**（review）：直接进入 `world/review`，工作台内以
...
- 因此「需要决定」入口上的总计数角标**必须常显**（为 0 时整个角标隐藏，见 §4.1），
  使用主规范 §2 强调色使用规范第 2 条「待处理计数角标」——这是 world 页唯一允许使用
...
- 「需要决定」是直达统一工作台的 subnav 项，总计数用 §5.8 待处理角标（朱红小圆点 +
...
> 是全产品「AI 产出 → 待处理 → 人工采纳」模式的**基准实现**。outline 的大纲建议与
> scene 的场景候选项等同类队列必须复用同一套 class 语义与
...
- **批量操作**：三队列 scope 独立的 `.bulk-toolbar` 保留；批量按钮继续走 confirmAction
  ...
  否则在请求前阻断。批量请求晚到时仅在发起作品与当前页面仍匹配时更新草稿、选择、回执和
  提示；切换作品或离开子页后静默丢弃。单条采用成功后先把下一条 `review_item` 写入 URL 再刷新，新组件直接
...
- **关系筛选契约**：列表查询新增可选布尔参数 `has_reverse_candidates`（该有向组存在
  ...
  与实际页内容一致。
- **计数语义**：一级「需要决定」、工作台类型 tab 与 Writing Home / Today 类型提醒均统计
...
- `aria-current="page"`：全部 subnav 项（一、二级）；aliases 深链继续把“资料库”标为当前页。
...
资料库首屏、搜索与翻页只消费轻量目录，打开具体页才读取正文；辅助资料按入口加载。长期作者可在世界模型面板就地修改本轮变化，查看当前保留/放弃/待定决定，并从完整历史选择参考。历史成果单独开启会话，避免与当前模型混用。

影响条目显示声明关联或可能提及，打开时校验具体版本；返回保留清单位置。语义范围逐项选择，未覆盖不显示为无问题。验证包括 390/768/1440px、键盘、失败/冲突、离开恢复与合成长文库；喜好和重复使用仍需作者试用。

世界与写作共用的 AI 抽屉通过 Vue Teleport 挂载到 body，位于侧栏之上、顶栏之下，避免工作区层叠隔离遮住平板操作。响应式验收须检查标签与收回按钮的实际命中元素，不能只依赖无横向溢出；截图同时覆盖抽屉入口和滚动后的模型编辑区。
