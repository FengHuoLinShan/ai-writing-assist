...
当前作品设置
├─ 创作偏好（默认页签）
└─ 高级导入（专家页签，参数整体默认隐藏，展开后再按作者问题折叠）
```
...
- 高级导入默认只显示作者摘要（系统/账户默认，或「当前作品有 N 项与默认不同」）与「查看专家参数」；
  不把覆盖对象交给通用来源组件，也不显示 raw JSON。全部参数整体隐藏，校验错误会自动展开并
  聚焦对应分组。只改渐进展示，不改任何请求 key、默认值或保存 payload。
...
- 浅／深色视觉基线覆盖账户页、项目页、两个项目页签及 390px 账户/项目页面。
