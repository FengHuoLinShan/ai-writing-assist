...
- **诊断分层**：概览、查找质量、状态警告与任务进度直接可见；技术信息、检索记录、重新连接、失败片段重试和最近片段宽表收在「诊断详情」内。运行状态以「后台连接／语义匹配耗时」表达；配置异常只提示联系管理员，不要求作者理解 worker、embedding 或重启后端。警告会自动展开，清除后不抢回作者的开合选择。
