...
> 覆盖 `#workbench/<id>/rag` 的两个路由子页：search（查找）与 status（索引修复）。search
> 是默认任务页，不设置内容级 tab；status 只从降级修复入口或深链进入，并在修复卡内返回查找。
...
| 5 | 中 | status 页信息埋藏过深：重建范围表单（`#rag-rebuild-content-mode/start/end`）藏在「诊断详情」details 底部，主操作「修复查找功能」在 details 外，点修复时使用哪个范围不可见；「返回查找」仍有两处 | `components/RagStatusPanel.vue` |
...
- **入口**：search 不设置内容级 tab；status 仅由 search 的降级通知条「修复查找功能」或
...
- **返回**：status 内既有 `nav-search`「返回查找」入口保留；本次只移除页面顶部的单项 subnav，不顺带整理 status 内部。
...
`role="status"`（降级通知条）；`role="alert"`（检索错误卡、章节范围错误）；`aria-label="检索关键词"`；`aria-label="近期检索记录"`；章节范围 `aria-invalid`/`aria-describedby`；`role="progressbar"`（WorkflowProgressCard）。**新增**：抽屉 `role="dialog"` + `aria-modal` + `aria-label`。
...
4. 重建范围表单在「修复查找功能」按钮旁可见；status 内部返回入口的后续整理不属于本次改动。
