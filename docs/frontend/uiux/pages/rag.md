...
- **诊断分层**：概览、查找质量、状态警告与任务进度直接可见；技术信息、检索记录、重新连接、失败片段重试和最近片段宽表收在「诊断详情」内。运行状态以「后台连接／语义匹配耗时」表达；后端警告逐条投影为受影响范围与作者可执行动作，不透传配置常量。最近片段状态统一为「已完成／整理中／等待整理／失败／未知状态」。配置异常只提示联系管理员，不要求作者理解 worker、embedding 或重启后端。警告会自动展开，清除后不抢回作者的开合选择。
