...
- **按契约触发**：普通业务、基础设施和前端实现变化仅提示复核，不要求格式化无影响说明；
  API、schema、facade、路由、任务协议等稳定边界变化仍触发硬门禁，不能省略必要文档。
...
- 12 个业务模块（含本机设备模块和 Evolution 契约层）、共享基础设施、前端与 Prompt 的代码根、设计文档和代码邻近 README；
- 每个后端组件的 API 前缀；
- 代码路径到必查文档的差异影响规则。
...
1. **完整性门禁**：无须 Git base。验证清单文件存在、模块目录与模块文档一一对应、所有 ORM
   表在数据库权威目录中可见、API 前缀/任务/路由/Prompt 已登记、ADR 全部进入
...
2. **差异影响门禁**：提供 `BASE_REF` 后，根据 API、facade、contracts、ORM、migration、
   task、前端 route/wire 和 Prompt 的实际改动，列出本轮必查文档；未修改的必查文档必须
   在 PR 模板中逐项核对并说明无影响原因。普通实现规则使用 advisory 提示，不阻断。
...
硬门禁列出的文档是**必查面**，不是强制改写面。如果答案均为“否”，在 PR 模板勾选“已逐项
