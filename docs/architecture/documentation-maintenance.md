...
验证证据同步到正确位置。它适用于代码已经完成、准备验收或准备合并的开发轮次；小型
内部重排不应为了形式制造文档噪声。
...
| 设计取舍改变且会约束未来实现 | `docs/adr/` | 需要长期架构决策时；先取得用户确认或走 ADR 流程 |
...
如果答案均为“否”，在 PR 中注明“已检查，无当前文档影响”即可；不要为了凑更新而改写
文档。
...
- 已更新的当前文档：...
- 未更新原因：无影响 / 历史记录不回写 / ...
- 验证：测试命令；`git diff --check`；必要的术语或链接检查
```

当文档影响无法在本轮完成时，必须写明阻塞原因、目标文档、责任人和最晚处理的后续项；
如果变更已经改变当前用户语义或稳定契约，则不能带着该缺口合并。
...
- **跨模块改动的负责人**：维护 `docs/00_整体设计.md`、`CONTEXT.md`、数据库设计和架构图中
  受影响的部分，并协调各模块说明。
- **评审者**：以实际 diff 对照影响矩阵，拒绝“公共行为已变、文档以后再补”的交付。
- **合并负责人**：只确认本轮文档闭环和索引可达性，不把历史计划误改为当前设计。
