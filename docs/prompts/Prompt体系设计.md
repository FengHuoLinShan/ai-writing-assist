...
| `interaction/prompts.py` / `evidence/compilation/services/interaction_story_context.py` | 内联 `interaction-story-v3`：兼容模型知识 RP，source-bound 旅程额外注入版本/截止点经 Evidence 校验的作品参考块与可选隐藏尾部元数据 | interaction 故事任务 |
...
`interaction-story-v3` 直接输出可见故事。正文之后可以有一个带固定边界标记的可选 JSON
...
v3 的事实优先级固定为：用户最新明确修正 → 当前选中旅程历史与手工回顾 →
当前绑定 source revision 且截止点前的作品资料 → 模型训练知识。source-bound 路径禁止用
训练知识填补原作设定或未来剧情。Evidence 关闭 LLM 查询规划/重排，只用确定性别名匹配、
exact manifest 检索和 Writing 回读；参考块纳入既有 RP 输入硬上限。快照只保存 hash、引用、
原因码和预算摘要，不保存原始对话或长期 rendered source block。
