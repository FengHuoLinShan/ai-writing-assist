# 世界书系统增强计划（2026-08-13）：从「真名回响」Wiki 详细程度反推需求、核对现有能力的增量计划
...
> 与既有计划的关系：本计划是 2026-08-10 世界书重构计划的第一轮增强——不另起炉灶、不推翻既有 7.x 定案；把 wiki 新证据中三版设计达成共识的缺口以零迁移薄读形态挂入既有 seam，P1 用窄字段增强接线，全部新 schema 押后至触发条件＋ADR。
> 方法：双工作流多代理——Workflow A（8 个侦察代理并行：wiki 4 维度需求侧＋系统 3 维度供给侧＋既有计划覆盖图，1 个合成代理出差距矩阵）；Workflow B（3 个设计代理：最小改动优先／生成质量优先／作者治理优先，1 个判定代理合成最终计划）。
> 中间产物：`docs/references/data/2026-08-13-worldbook-gap-analysis.json`（需求 41 条／供给 29 条／差距 30 条）、`docs/references/data/2026-08-13-worldbook-final-plan.json`（三版设计与合成计划）。
...
5. **P1 窄增强共 14 项，仅 3 项涉及可选字段扩展**（CharacterKnowledge 四字段、ConflictCheckQueueItem 三字段、source_anchor），全部向后兼容、旧数据默认值语义一致；其余为读模型与 Prompt 纪律。
6. **P2 全部触发式**：6 项新 schema（术语注册表、统一待决队列、影响审查生命周期、世界书包存储、Decision Ledger、规则与 R 门台账）一律押后至 ≥3 真实项目证据或 08-10 计划 8.2 五条门槛同时成立＋用户裁定＋ADR；真名回响单项目 wiki 证据不构成新 schema 触发条件。
7. **1 条设计分歧需用户裁定**（统一待决队列：P0-7 维持 7.4 路由方案为默认，P2-2 统一队列为备选）；其余 6 条开放问题已有明确推荐（见第 8 节）。
...
按严重度排序。关系标注：not-covered=08-10 计划未覆盖；already-planned=已覆盖（标注 7.x）；needs-refresh=已覆盖但证据需刷新。
...
| 13 | medium | partial | 已计划 7.4（设计分歧见第 8 节） | B4 待定项唯一权威队列(P0/P1/P2 阻塞分级+最小回执) | 多队列并存(suggestion/conflict/dedup)无优先级分级、无单一入口(code)；计划 7.4 明确以运行时路由替代统一队列 |
...
#### P0-7 统一待决入口的阻塞分级投影（维持 7.4 路由方案）

- **覆盖需求**：B4
- **与 08-10 计划关系**：延续 7.4＋承接 open_questions#1（默认维持路由方案不造统一队列，补 P0/P1/P2 阻塞分级与「已锁定|仍需裁决」两列语义）
- **实现形态**：前端编排（只读合成视图，阻塞级为计算投影不落库）
- **范围**：Today「需要你决定」与世界书审查页为各领域待决项补阻塞分级投影（源于各领域既有状态，不聚合持久化）与最小回执模板（已锁定/仍需裁决）；批量采用与逐条采用继续走 7.3 收束卡与现有同类型批处理
- **理由**：B4 是设计分歧；两版 P0（D1 P0-9/D3 P0-2）、D2 因 open_questions#1 暂缓——裁决：维持 7.4「不造统一校验队列」结论（避免新 schema），先以纯前端投影承接 wiki 阻塞分级与两列语义；若真实回放证明投影不够（pending 持续污染待办、跨设备丢失裁决语境），升级 P2-2（需用户裁定）
- **验证**：R05 扩展断言（候选≠错误、延期不污染待办、P0 阻塞项醒目、批量采用后队列收缩）；窄屏与空态走查
- **风险**：若用户最终裁定改向统一队列，本项存在返工面——但投影在裁定前已积累真实测量，返工有限
...
### 5.2 P1 窄增强接线（14 项）——仅 3 项涉及可选字段扩展，全部向后兼容
...
### 5.3 P2 触发式 schema（6 项）——全部押后至真实项目证据＋ADR
...
#### P2-2 统一待决队列权威 schema（阻塞分级＋最小回执生命周期）

- **覆盖需求**：B4
- **与 08-10 计划关系**：替代 7.4 路由方案（设计分歧 open_questions#1，需用户裁定后定案）
- **触发条件**：触发条件：用户裁定改向＋≥3 真实项目证明 P0-7 投影无法承载跨设备阻塞分级与回执生命周期（pending 持续污染待办、作者丢失裁决语境）；wiki 单入口＋阻塞分级是证据但单项目不满足 4.2 新结构门槛；P0-7 投影先上线积累测量
- **范围（触发后）**：单一权威审查入口：P0/P1/P2 阻塞分级、「已锁定|仍需裁决」两列、最小回执模板、「首次进入剧情再审」延迟触发、跨设备持久生命周期
- **验证**：ADR 先行（deletion test、领域所有权、迁移）；回放夹具（延期项不污染、升级路径可审计）
- **风险**：用户裁定前不预建——若裁定改向，P0-7 投影存在局部返工
...
- 不新建统一校验队列、Decision Ledger、依赖图表、独立术语表——4.2 边界维持，全部押后至 P2 触发条件（≥3 真实项目证据或用户裁定）＋ADR；单项目 wiki 证据（真名回响）一律不构成新 schema 触发条件。
...
P0 内部依赖链：**P0-1 作者闭环**（延续 08-10 首批切片，一切门禁的宿主）→ **P0-2 术语状态约定**（数据源）→ **P0-3 质量 lint 三合一**（消费术语状态）→ **P0-4 规则条目约定**（数据源）→ **P0-5 触发预检**（消费规则条目与 P0-8 未闭合项）→ **P0-6 负面边界** → **P0-8 变更记录模板** → **P0-7 阻塞分级投影**；**P0-9 回放夹具**（R15—R17）伴随各 P0 项验收同步补。

P1 内部依赖链：**P1-2 时点门禁**（7.14 纯复用接线，最早可启动）→ **P1-3 知识账字段** → **P1-4 术语矩阵**（依赖 P0-2 契约升级与 P1-3 的 R13 编译消费）→ **P1-5 冲突队列窄增强** → **P1-1 跨层失效投影** → **P1-7 依赖边约定**（消费 P1-1 读模型）→ **P1-8 漂移检查器**（接通冲突队列真实生产者）→ **P1-9 社会真实度审查卡**（依赖 P0-8 回执宿主）→ **P1-6 复杂度画像与项级控制** → **P1-10/11/12/14** 低优先级薄读补丁 → **P1-13** 08-10 计划 P1 切片延续（7.5/7.7/7.9/7.10/7.12，独立轨道）。
...
| # | 问题 | 决议（推荐方案） | 落点 |
|---|---|---|---|
| 1 | 统一待决队列分歧：wiki 要求「唯一权威审查入口+P0/P1/P2 阻塞分级+最小回执模板」，08-10 计划 7.4 明确「不造统一校验队列」以运行时路由替代——维持路由方案（需补 P0 阻塞级语义）还是允许轻量队列读模型？ | **需用户裁定**：维持 7.4 路由方案：P0 以纯前端投影补 P0/P1/P2 阻塞分级与「已锁定|仍需裁决」两列语义（P0-7，不落库）；只有 ≥3 真实项目证明投影无法承载跨设备阻塞分级与回执生命周期时才走 P2-2 统一队列（需 ADR）。 | 默认 P0-7 投影／备选 P2-2 统一队列 |
...
注：正文中 open_questions#N 编号与本表一致。第 1 条为唯一需用户裁定的设计分歧。
