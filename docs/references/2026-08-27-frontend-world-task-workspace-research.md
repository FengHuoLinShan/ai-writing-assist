...
### Phase E2：Lo-fi 资料入口重排

Phase E2 根据内部 Lo-fi 反馈把固定资料目录和顶部常驻工具面板收敛为一级侧栏内的动态工具卡，资料库首页先显示常用六类、工作稿和更多类型，再进入现有 tagged Card 结果页。工具卡按资料库、关系和需要决定上下文切换；移动端不改变五项底部导航，只提供页面内工具弹窗。世界健康、页面中的未决项和更多类型复用 Vue 模态焦点边界，高风险保存／发布／采用仍留在内容区。统一创建先区分具体对象与资料页，人物详情只补现有 Character 简单字段，不建立第二 Card、类型或事实源。后端 API/schema/Canon wire 不变；类型数量复用现有 Entity facets 与完整 Page/Draft 列表。
...
- 当前实现影响：Phase A–D、E0–E2 已落地，包括世界资料库类型首页、一级侧栏动态工具卡、位置恢复、旧深链/对象工具兼容、轻量作者任务、写作首页摘要、四类来源入口与受控试用自动化。真实作者试用尚未执行。
