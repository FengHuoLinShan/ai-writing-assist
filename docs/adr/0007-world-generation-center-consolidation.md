...
- 生成中心 world 工作区承载来源、目标、对话、上下文、目标配置和结构化预览；本地状态以
  项目、来源页面和目标隔离。服务器页面正文和服务器工作稿仍是唯一权威来源，绝不缓存到本地。
  仅允许保存一个版本化、受 512 KiB / 最多 5 个会话边界约束、精确绑定 pending suggestion 的
  作者“未应用提案编辑”工作副本；它不是页面/工作稿副本，不可冒充 canonical，成功应用或作者
  明确放弃后清理。
