...
| 世界书 | `world_bible_categories`, `world_bible_page_drafts`, `world_bible_pages`, `world_bible_page_revisions`, `world_bible_page_projections` | 自定义类别、服务器工作稿、发布页、版本快照和可编译投影分离；页面/工作稿可保存有稳定 ID 的 `sections_json` 和 `page_meta_json`，发布以 `base_version_number` 做 CAS。导入来源的 source key/hash/baseline 随工作稿发布与修订恢复传播。 |
| 世界书校验 | `world_validation_runs` | 按 `novel_id` 保存冻结的 policy/manifest/dependency/target hash、ReviewPacket 输入/结果 hash、coverage/budget 账本、verdict/gate 和 warning 签收。`task_id` 唯一关联 `async_tasks`；部分唯一索引只限制同项目的一个 `full + queued/running`，不阻断 targeted run。项目永久删除时 CASCADE，task 删除时仅 SET NULL。 |
...
| 世界事实权威 | `world_assertions`, `world_canon_revisions`, `world_canon_heads` | Assert 与完整 Canon manifest/receipt 不可变；每项目唯一 head 只以单父 CAS 前进。empty C0 不提升 legacy 数据；页面发布只替换 exact documentary revision，Name 写入与显式 B promotion 才创建 selected Assert。普通 UPDATE/DELETE 由 ORM 与 PostgreSQL trigger 拒绝，项目永久删除才级联。 |
| Profile Schema 历史 | `entity_profile_template_revisions` | 现有 `entity_profile_templates` 稳定 head 的 exact immutable revision；typed field/relation promotion 必须引用其明确声明，新 Profile 固定 exact revision，不与页面布局模板混用。 |
