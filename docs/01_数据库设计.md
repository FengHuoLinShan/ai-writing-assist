...
| 表 | 设计要点 |
|---|---|
| `map_atlas_runs` | 一次计划/生成任务的授权、secret-free 快照、来源 hash、计划、进度与停止状态。 |
| `map_atlas_nodes` | 跨 run 复用的层级节点；新节点 provisional，采用页面时连同祖先原子采用。 |
| `map_atlas_pages` | 独立候选/已采用/拒绝/移出图片；必填项目、run 和 node，派生历史用自引用表达。 |
...
旧动态地图 17 表由 `20260812_ai_map_atlas` 迁移直接删除；demo 阶段不做数据兼容。
