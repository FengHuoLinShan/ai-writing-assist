...
| 表 | 归属 | 设计要点 |
|---|---|---|
| `evolution_runs` | evolution | `(novel_id, run_key)` 唯一的理解推进运行注册表：owner epoch（切换/重建推进，旧 epoch 拒绝提交）、已提交前缀游标（committed prefix，只在回执落库的同一事务内推进）、head 指针与根预算（`budget_remaining` 条件 UPDATE 原子预留，T21 不透支）。 |
| `evolution_frozen_attempts` | evolution | prepare 阶段冻结的窄批次负载（模型返回后的完整待提交内容，T10 恢复基础）；同 attempt 重复冻结且 manifest 不同则拒绝。 |
