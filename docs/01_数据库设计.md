...
统一 AI run 不新增列：队列投影使用 `async_tasks.meta._ai_run_envelope`，Interaction RP
的权威 run 同时镜像到 `interaction_generation_attempts.agent_checkpoint_json._ai_run_envelope`。
续段换 task 时保留 attempt `run_id`、累计请求数、授权版本和原 deadline；旧 task 终态不回写
新 task 已接管的进度。这些下划线 JSON 均为内部载体，不进入公开 task wire。
...
| `assistant_editorial_reviews` | `novel_id` 隔离的作者编辑任务；`(novel_id, operation_id)` 唯一，冻结工作稿清单、编辑约定、模型快照、分段进度、遗漏和历史报告；项目永久删除级联删除。 |
| `assistant_editorial_issues` | `novel_id` 与问题指纹唯一的编辑建议、引文、作者决定、旧证据和改后复核；`(novel_id, review_id)` 复合外键防跨项目关联。不构成 Writing candidate 的采用回执。 |
...
| 表 | 归属 | 设计要点 |
|---|---|---|
| `accounts` / `account_identities` | account | 账号状态、支持码与唯一邮箱、Authing 微信或 `anonymous_rp` 身份；匿名账户以 nullable `temporary_expires_at` 标记 24 小时级联清理边界。 |
| `web_sessions` / `email_login_challenges` | account | 只保存会话令牌、CSRF、邮箱和验证码的 keyed HMAC 摘要；匿名 RP session 同样不保存 provider Key。 |
...
| `demo_project_copies` | project | `(owner_id, source_project_id, source_version)` 唯一地指向一个 owner 自己的可编辑演示副本；源或副本永久删除时级联清理标记。 |
...
| `story_simulation_runs` | story | 有限排演请求、来源 hash、预算、完成预览和可选父排演；与原 one-click task 共用 ID。 |
| `story_simulation_steps` | story | 同项目 run + round 唯一的不可变意图、裁决事件、观察状态和输入/输出 hash；分叉复制已完成前缀。拒绝 UPDATE/直接 DELETE，允许父对象永久删除级联。 |

Story 复用 `async_tasks` 承载 card/reaction/script/one-click 四类任务，排演 run 是领域回放记录，不新增队列。所有 API
...
### 3.9 写作、异步任务与本机 Agent（8 表）

| 表 | 归属 | 设计要点 |
|---|---|---|
| `writing_drafts` | writing | 正文版本事实源；保存 content hash、冲突检查快照、生成/采用 provenance 与不重排版本号。`editorial_ready_at` / `editorial_ready_hash` 记录作者把某一已保存工作稿交给编辑的意图，不等于发布或采用；改稿后旧 hash 不代表当前正文。普通 working 选择排除未采用 candidate；采用时复制为新 draft 并把原建议转入历史。published 不可原地修改，常规删除改为 deprecated。 |
| `writing_comments` | writing | 作者和 AI 审稿批注绑定具体项目、草稿版本、整篇与选区 hash、码点区间；审稿位置无法唯一定位时保留无区间评论。跨版本只显示失效状态，不猜测新位置；来源审稿 task/finding 去重，最后修订 task 仅作结果导航。 |
| `writing_conflict_checks` | writing | 对某个草稿/场景/章节范围的冲突检查运行；scope.content_hash绑定正文，summary_json.ai_review.recorded_at记录语义复核回执时间，同稿历史可按版本、内容hash与候选范围筛选。 |
| `writing_conflict_items` | writing | 逐项证据、严重性、AI 判断与建议处理状态。 |
| `async_tasks` | infrastructure/tasks | 全局任务队列；使用五状态、heartbeat、lease/attempt/recovery policy、进度与 result/meta。`novel_id` 是可索引、不可变的一等项目边界并外键级联到 `projects`；`meta.novel_id` 只是受 trigger/ORM 校验的兼容投影，全局任务两者皆为空。内部 `coalescing_key` 对规范化项目、task type 与有序 scope 的 canonical JSON 做 SHA-256；数据库分别限制每个 key 最多一个 pending 和一个 running，终态保留历史。ADR-0013 的作者长任务复用 UUID 主键作为 operation receipt，并在 meta 保存不含 secret 的请求指纹；相同项目/类型/指纹恢复原终态任务，不新增表或索引。World 精细设计将稳定阶段响应和审查回执写入 result 的下划线私有键，输入 hash 一致时供自动重排复用；公开 task wire 过滤这些键。API 一律以列隔离访问，内部 key/指纹不进入作者界面。 |
| `local_agent_devices` | local_agent | `owner_id` + `novel_id` 绑定 Mac 设备；只存配对码和设备令牌摘要、有效期、最近心跳与撤销时间。配对码摘要唯一，十分钟单次使用。项目设置只保存所选 CLI 与设备 ID。 |
| `local_agent_invocations` | local_agent | 每个原 task 的 Agent 调用按 `(task_id, ordinal)` 唯一；冻结设备、CLI、请求与原 task lease。伴随进程领取后生成独立 invocation lease，回执保留状态、已见文本与未知用量；任务停止、租约替换时拒绝迟到结果。 |
| `local_agent_tool_calls` | local_agent | `(invocation_id, call_id)` 唯一，保存本轮已注册产品工具的参数、pending/running/completed/failed 状态与结果；先持久化 running 再执行，崩溃不自动重复工具副作用。 |

### 3.10 RP 互动旅程（9 表）

| 表 | 设计要点 |
|---|---|
| `interaction_source_revisions` | 同 owner author 项目的不可变资料目录；`(source_novel_id, version_number)` 和 `(source_novel_id, manifest_hash)` 唯一，保存 draft/hash manifest、锚点、对象目录、歧义决议、workflow/task、就绪状态和 ready 整体 fingerprint，不复制全文。ADR-0024 仅将配置精确指定且复验通过的一条 revision 只读提供给匿名 consumer，仍不复制全文或改变普通 owner 规则。source project 删除时级联，但旅程对 revision 的 RESTRICT 会在有引用时阻断。 |
| `interaction_journeys` | 一个隐藏 interaction 项目恰好对应一个旅程；可空冻结 source revision、章节内锚点、玩家身份、固定/忽略策略与 source context epoch；另保存 owner、旅程开关、generation_mode=standard/ensemble、当前选中叶和回顾 head/epoch。 |
...
| `interaction_actor_state_revisions` | 与正式消息节点同事务写入的派生人物观察；复合 novel/journey/node/parent 外键防串项目及旅程；读取只沿选中祖先和冻结 source revision，不按最新时间选分支。历史 UPDATE 和直接 DELETE 被数据库拒绝；父对象永久删除的级联保留。 |
...
### 3.11 演化运行注册表、冻结尝试与回执（3 表，V4 E04）

| 表 | 归属 | 设计要点 |
|---|---|---|
| `evolution_runs` | evolution | `(novel_id, run_key)` 唯一的理解推进运行注册表：owner epoch（切换/重建推进，旧 epoch 拒绝提交）、已提交前缀游标（committed prefix，只在回执落库的同一事务内推进）、head 指针与根预算（`budget_remaining` 条件 UPDATE 原子预留，仅 active run 可扣减，T21 不透支）。另有部分唯一索引 `uq_evolution_run_single_live_writer`（`novel_id` WHERE `execution_mode='live' AND status='active'`）：同项目同时至多一个 active live run 的数据库级不变量，count-then-insert 竞态在索引层只有一个赢者；排空/停止的 run 不因重复注册复活。 |
| `evolution_frozen_attempts` | evolution | prepare 阶段冻结的窄批次负载（模型返回后的完整待提交内容，含来源绑定与编译后观察，T10 恢复基础）；同 attempt 重复冻结且 manifest 不同则拒绝。 |
| `evolution_receipts` | evolution | 已领域提交回执的追加式记录（T11 重放依据）；同 attempt 不可改写；保存时同事务推进 run 游标并把冻结尝试标记 applied。 |

三表均带 `novel_id` 外键级联；repository 一律以 `(novel_id, run_key)` 作用域读写。
evolution 复用现有 `async_tasks` 与项目 LLM 入口，不新增队列。注册的
`evolution_scene_step_v2` 可执行显式任务，但默认导入入口仍由 deep_import 编排；
项目级旧/新 owner 现由 projects 的引擎/epoch/schema floor 和 PG trigger 共同约束，
不能只依据单个 live run 唯一约束判断写入权。
...
`20260913_schema_parity_repair` 幂等修复旧 live-metadata baseline 可能遗漏的
StoryOutline 项目隔离外键、不可变触发器与作者待办输入约束；迁移前须先确认
存量数据没有跨项目引用、空标题或超长备注。
...
持续共创补全继续使用已有表：模型变化落在 suggestion 的 world_design_checkpoint.v1 JSON，当前指针与成果消息在同一会话事务中推进；world_cocreation_turn 的请求指纹、固定 LLM snapshot、精细设计阶段响应和完整审查回执使用 async_tasks 既有 JSON result。checkpoint 只保存精简审查引用；作者编辑后的结果明确标为未复核。validation run manifest 保存 semantic_items、确认/范围指纹、review_domains/depth 与无正文的 Focused Evidence 回执。没有新增表、列或 migration。
...
本轮不新增表或 migration。scope/director/audit 指纹与脱敏 `knowledge_review` 仅写现有 confirmation compile options、context snapshot metadata、task result/checkpoint 和领域 provenance JSON；隐藏全文、Prompt 和密钥不持久化。

有限协作两条新增迁移保留对称 downgrade，删除新增历史表、模式字段、复合唯一约束与触发器函数。
该回退会丢弃新功能历史，仅用于已明确授权的可丢弃环境；日常应用回退关闭功能并保留 schema/data，见 development-guide。

## 技术覆盖实验的存储边界

父级前后文沿用不可变 Writing draft 与 Scene span，不新增父级文本表；工具历史版本
沿现有 checkpoint JSON 保存。SSE 恢复仍以 attempt 的持久正文与 Unicode 字符 offset
为准，不新增事件表。BM25 与 MCP 合成实验数据及评测产物属于开发工具，不进入业务事实表。

## 前瞻与创作试验的派生记录

| 表 | 所有者 | 用途与约束 |
|---|---|---|
| `assistant_forecast_candidates` | Assistant | 不可变单次评估；事项身份、主体、上下文、评估摘要与到期时间 |
| `assistant_forecast_dependencies` | Assistant | 同项目候选的完整来源/范围/授权依赖，不只保存引用命中 |
| `collaboration_cases` | Collaboration | 作者目标及历史、Grant 及历史、配方、累计请求消费 |
| `collaboration_runs` | Collaboration | 固定任务身份、代次、来源 manifest、连接快照与预算 |
| `collaboration_work_items` | Collaboration | `(run_id, logical_key, generation)` 唯一的依赖图工作项 |
| `collaboration_artifacts` | Collaboration | 不可变带来源成果，含精确工作区检查 |
| `cognition_heads` | Collaboration | 每 novel_id/scope 唯一的可变理解 head，commit 复合外键与 generation CAS |
| `cognition_commits` | Collaboration | 独立于 run 的不可变操作回执；novel_id/operation_id 唯一、父提交、方法、完整读集与来源；no_change 不推进 head |
| `cognition_records` | Collaboration | 独立理解修订：稳定 record_id、正文/根来源/传递认知/查询依赖、evolution_refs_json 原观察回执、hash、作者状态；部分唯一索引限定每项目每记录唯一 current；PG 仅允许 current 退役，禁改写历史内容 |
| `creative_workspaces` | Collaboration | 冻结基线、父工作区、当前修订和请求幂等摘要 |
| `creative_workspace_revisions` | Collaboration | 不可变累积覆盖、父修订、目标版本与完整 digest |
| `creative_merge_receipts` | Collaboration | 不可变精确采用回执；操作身份与规范请求摘要绑定 |
| `domain_outbox` | Collaboration | 和领域修改同事务产生的投影失效投递记录 |

关联通过含 novel_id 的复合外键约束；不可变规则由 PostgreSQL trigger 执行。
理解历史无 run 级联外键，运行保留期不删除理解；项目永久删除级联清理三表。
`20260922_cognition_evolution` 为理解修订追加非空列表 `evolution_refs_json`；其依赖与
正文一起受不可变历史保护。commit provenance 留存实际作者目标、约束与工作输入，
明确作者回顾范围，不能用仅有 learned_at 的时间标签证明历史阅读合法。
`20260922_evolution_model` 增加 `evolution_runs.llm_snapshot_json`，首次入队冻结
非 secret 连接摘要，同 run 续步复用。历史 NULL 不伪造旧配置；已冻结结果可恢复，
需要新请求的旧运行须明确新建。密钥仅经账户入口在执行时读取。
`20260922_evolution_reading` 增加 nullable `evolution_runs.reading_plan_json`，保存
逐场景理解的固定来源范围、Scene 顺序、分段操作/授权与当前队列任务；不新建第二份游标
或预算。没有计划的历史 run 不猜补范围。已计划但尚未采样的章节同样纳入来源失效。
`20260922_evolution_invalidation` 增加 `memory_events.source_stale` 与
`evolution_runs.invalidation_json`：来源变化保留原机器事件与回执，标明待重算范围，
并推进 owner epoch 封锁旧提交；作者确认事件不标 stale。
`assistant_runs.operation_id` 与 `assistant_notices.row_version` 是 additive 字段；旧 notice
写入同样递增版本。`interaction_message_nodes.input_json` 保存输入类型和私语/参与者引用，
随不可变分支输入保留。四条 `20260920_*` migration 从当前 head 顺序升级；不改写旧 squash。

### 理解引擎迁移门禁

`20260922_understanding_owner` 给 projects 增加 `understanding_engine`（legacy/evolution/read_only）、
`understanding_epoch` 与 `understanding_schema_floor`，给 evolution_runs 增加 `project_owner_epoch`。
默认 legacy/1/1；迁移将已有 live run 的项目暂停为 read_only/2/2，保留其原始预算和回执。
建 guard 前取消全部旧 evolution_scene_step 队列项以及暂停项目的旧理解任务，并停止相应
活动 run；不删除结果、来源、checkpoint 或历史费用，不阻塞其他项目任务领取。

PG trigger 保证 epoch/schema floor 单调、切换推进 epoch、禁止 schema 2 降回 legacy；
AsyncTask/ImportWorkflowRun/EvolutionRun 写入需匹配冻结 token，不能改任务类型或替换 token
绕过门禁。旧 v1（含 shadow）均拒绝执行；新协议为 evolution_scene_step_v2。迁移前无 token
legacy 任务只在项目仍为 legacy/1/1 时可继续。存在 floor 2 项目时 downgrade 明确拒绝，
回退应暂停写入并保留兼容新版，不清空预算或把新版状态交给旧二进制。
