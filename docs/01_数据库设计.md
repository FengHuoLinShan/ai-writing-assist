...
| 世界书 | `world_bible_categories`, `world_bible_page_drafts`, `world_bible_pages`, `world_bible_page_revisions`, `world_bible_page_projections` | 自定义类别、服务器工作稿、发布页、版本快照和可编译投影分离；PageRevision 持久化 `revision_digest`，发布以 `base_version_number` 与 expected Canon head 双重检查，在同一事务中 seal revision、选入 CanonRevision 并 CAS 推进 head。 |
