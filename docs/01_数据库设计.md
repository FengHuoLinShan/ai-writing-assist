...
### 3.7 导入、检索、记忆与上下文（14 表）

| 子域 | 表 | 设计要点 |
|---|---|---|
| imports | `import_records`, `imported_chapters` | 导入元数据与章节正文分离；章节属于项目和导入批次。 |
| rag | `rag_chunks`, `rag_index_state`, `rag_entity_appearances` | 可重建检索块、novel/chapter/content-mode 级新鲜度状态，以及按 Scene 去重（无法映射时按章去重）的对象出场派生索引；appearance 保存 source hash，只消费每章最新新鲜 working、否则 canonical。 |
