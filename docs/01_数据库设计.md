...
| 表 | 设计要点 |
|---|---|
| `interaction_openings` | 同 owner 作者项目的 RP 开局卡；保存冻结 source_revision、JourneySourceSetup、开局文案、体验类型与明确审查的对象图片版本。调用方幂等创建仍由既有 Journey 承担。图片版本失效不回退，归档保留记录。 |
| `interaction_source_revisions` | 同 owner author 项目的不可变资料目录；`(source_novel_id, version_number)` 唯一，`(source_novel_id, manifest_hash)` 为非唯一检索索引，允许同正文更新已整理资料并冻结新版。保存 draft/hash manifest、锚点、对象目录、歧义决议、workflow/task、就绪状态和 ready 整体 fingerprint，不复制全文。无内容变化时复用最新匹配版本，旧旅程保持旧绑定。ADR-0024 仅将配置精确指定且复验通过的一条 revision 只读提供给匿名 consumer，仍不复制全文或改变普通 owner 规则。source project 删除时级联，但旅程对 revision 的 RESTRICT 会在有引用时阻断。 |
