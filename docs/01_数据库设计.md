...
| `account_llm_credentials` | settings | 按 `(owner_id, provider_id)` 唯一保存已真实验证的加密 API Key、指纹和验证时间；账号删除时级联。 |
| `global_llm_defaults` | settings | 按 `owner_id` 唯一保存当前账户模板/provider 及非 secret 兼容默认；不保存 API Key。 |
...
`account` 与 `settings` 分别是公开身份边界和 owner 级默认配置边界；settings 模型在
[`backend/modules/settings/models.py`](../backend/modules/settings/models.py)。新业务调用的
...
| 连续线路 | `map_path_layers`, `map_paths`, `map_path_nodes` |
| 时间/对象标记 | `map_markers`, `map_territory_tiles` |
| 动态事实 | `map_observations`, `map_facts` |
| 视觉历史 | `map_visual_revisions` |

`map_observations` 是带证据的观察层，`map_facts` 是采用后的地图事实层；
`(novel_id, observation_id)` 唯一约束保证同项目一条 observation 只生成一个逻辑 Fact。
两者通过时间锚点、
空间锚点、证据和 Scene/章节来源保持可追溯；作者界面只把可操作 observation 显示为待处理，
不会为了统一文案压平两张表。`MapDelta`、Scene 状态切片、连续性问题和 `WorldDynamic`
由 world/map 在读取时从 confirmed `map_facts` 确定性派生，不新增第二套事实表，也不在 GET
请求中回写状态。observation/fact 的地图、状态和 Scene 组合索引用于支持有界时间线查询，
动态地图表数为 17。

`map_configs.status/archived_at` 记录地图归档状态，active 根地图和 active 同父子地图分别由
PostgreSQL partial unique 约束名称；归档历史可与新地图同名。`editor_revision` 是视觉编辑
CAS 版本。`map_visual_revisions` 按 `(map_id, revision_number)` 保存不可变提交状态和资源级
正向/反向变更；恢复生成新 revision，不覆盖旧历史。地点绑定格、势力范围格、tile 与 terrain
patch 可作为当前投影删除，但删除前后的完整行仍进入该历史。`map_layer_nodes` 是递归图层局部显隐、锁定、透明度、排序和 zoom 区间的权威，
terrain 图层同名字段只作为旧接口兼容投影。`selection_mode/floor_level` 描述持久化的
normal、exclusive 与 floor 结构，但当前子层和 isolate 只存在于前端会话。
`map_path_layers` 只拥有 transport/water 类别，显示名称由对应图层 leaf 拥有；
`map_paths.status/content_revision` 保留线路归档身份和事实引用版本，节点表保存有序连续
轴向坐标。地图归档保留整棵子树及其视觉资产。
...
| `scenes` | outline | 最小叙事单元；保存逻辑顺序、结构字段、旧 `scene_chunks` 映射和 `structure_meta`。Planned Scene 允许空映射并以 `planning_state=planned` 区分正文定位损坏。 |
...
| rag | `rag_chunks`, `rag_index_state`, `rag_entity_appearances` | 可重建检索块、novel/chapter/content-mode 级新鲜度状态，以及按 Scene 去重（无法映射时按章去重）的对象出场派生索引；index state 以 `active_task_id + generation` fence 旧 attempt，appearance 保存 source hash，只消费每章最新新鲜 working、否则 canonical。 |
| memory | `memory_events`, `memory_scene_checkpoints`, `memory_scene_snapshots`, `memory_snapshots`, `delta_log` | Scene 事件溯源、分维度 checkpoint、稀疏全量快照与字段级变化日志；`memory_records` 已移除。 |
| context | `context_confirmations`, `context_confirmation_asset_refs`, `context_snapshots`, `evidence_links`, `context_retrieval_traces`, `context_activation_profiles`, `context_activation_profile_revisions` | 用户确认、确认记录精确资产引用索引、自动工作流审计快照、TargetRef provenance、检索运行摘要和版本化激活规则分离；运行时只消费已发布 Profile revision，confirmation/snapshot 固定实际规则与来源 hash。confirmation 的 JSON `compile_options` 同时保存实际检索锚点和作者请求的目标章节，防止跨章 Scene 误复用。 |
...
| `async_tasks` | infrastructure/tasks | 全局任务队列；使用五状态、heartbeat、lease/attempt/recovery policy、进度与 result/meta。内部 `coalescing_key` 对规范化项目、task type 与有序 scope 的 canonical JSON 做 SHA-256；数据库分别限制每个 key 最多一个 pending 和一个 running，终态保留历史。API 仍必须以 `meta.novel_id` 隔离访问，key 不进入公开 wire、错误或日志。 |
