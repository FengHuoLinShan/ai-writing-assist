...
describe("AI 地图册工作台", () => {
  ...
  it("没有图片或生图任务的手工地图可改标题并保留并发基准", async () => {
    ...
    await flushPromises()
    expect(wrapper.findAll('.atlas-node-form select')[1].findAll('option').map(option => option.attributes('value'))).toEqual(['region', 'city', 'district', 'street', 'interior'])
    ...
    expect(wrapper.find('.atlas-generation-settings').exists()).toBe(false)
    await wrapper.get('.atlas-rp-entry').trigger('click')
    expect(router.navigate).toHaveBeenCalledWith('journeys', 'new', true, expect.any(URLSearchParams))
    expect(router.navigate.mock.calls.at(-1)[3].get('project_id')).toBe('novel-1')
    editor.vm.$emit('state', { nodeId: editor.props('node').id, dirty: true, focused: true }); await flushPromises()
    expect(wrapper.get('.atlas-rp-entry').element.disabled).toBe(true)
  ...
  it('已采用图片对照默认关闭，左右保持不同且退出后释放另一张图片', async () => {
    const pages = ['a', 'b', 'c'].map(id => page({ id, title: '图片' + id, review_status: 'adopted', evidence: {} }))
    api.world.getMapAtlas.mockResolvedValue(tree(pages, 'atlas'))
    const wrapper = mount(MapWorkspaceView, { props: { projectId: 'novel-1' } }); await flushPromises()
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(1)
    expect(wrapper.get('.atlas-evidence').text()).toContain('未记录直接依据')
    expect(wrapper.get('.atlas-evidence').text()).toContain('未记录画面补全说明')
    expect(api.world.fetchMapAtlasImage.mock.calls.map(call => call[1])).toEqual(['a'])
    await wrapper.get('.atlas-compare-toggle input').setValue(true); await flushPromises()
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(2)
    const left = () => wrapper.get('select[aria-label="选择左侧图片"]')
    const right = () => wrapper.get('select[aria-label="选择右侧图片"]')
    expect(left().element.value).toBe('b')
    expect(left().findAll('option').map(item => item.attributes('value'))).toEqual(['b', 'c'])
    await left().setValue('c'); await right().setValue('b'); await flushPromises()
    expect(left().element.value).toBe('c')
    await right().setValue('c'); await flushPromises()
    expect(left().element.value).toBe('a')
    expect(right().element.value).toBe('c')
    await wrapper.get('.atlas-compare-toggle input').setValue(false); await flushPromises()
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(1)
    expect(URL.revokeObjectURL).toHaveBeenCalled()
    await wrapper.get('.atlas-compare-toggle input').setValue(true)
    await wrapper.findAll('.atlas-tabs button')[0].trigger('click'); await flushPromises()
    await wrapper.findAll('.atlas-tabs button')[1].trigger('click'); await flushPromises()
    expect(wrapper.get('.atlas-compare-toggle input').element.checked).toBe(false)
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(1)
    expect(api.world.reviewMapAtlasPage).not.toHaveBeenCalled()
    expect(api.world.updateMapAtlasAnnotation).not.toHaveBeenCalled()
  })

  it('移出当前或对比图片后归一化选择，只剩一张时关闭对照', async () => {
    const pages = ['a', 'b', 'c'].map(id => page({ id, review_status: 'adopted' }))
    api.world.getMapAtlas.mockResolvedValue(tree(pages, 'atlas'))
    api.world.reviewMapAtlasPage.mockResolvedValue({})
    const wrapper = mount(MapWorkspaceView, { props: { projectId: 'novel-1' } }); await flushPromises()
    await wrapper.get('.atlas-compare-toggle input').setValue(true); await flushPromises()
    api.world.getMapAtlas.mockResolvedValue(tree(pages.slice(1), 'atlas'))
    await wrapper.get('.atlas-review-actions > button').trigger('click'); await flushPromises()
    expect(wrapper.get('select[aria-label="选择右侧图片"]').element.value).toBe('b')
    expect(wrapper.get('select[aria-label="选择左侧图片"]').element.value).toBe('c')
    api.world.getMapAtlas.mockResolvedValue(tree([pages[1]], 'atlas'))
    await wrapper.findAll('.atlas-images figure')[0].get('button').trigger('click'); await flushPromises()
    expect(wrapper.find('.atlas-compare-toggle').exists()).toBe(false)
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(1)
    expect(wrapper.find('.atlas-images.compare').exists()).toBe(false)
  })

  it('切节点重置对照且拒绝旧节点晚到图片，新对比图失败仍可就地重试', async () => {
    const makeNode = (id, ids) => ({ id, title: id, level: 'city', children: [], pages: ids.map(value => page({ id: value, node_id: id, title: value, review_status: 'adopted' })) })
    api.world.getMapAtlas.mockResolvedValue({ mode: 'atlas', total_pages: 4, nodes: [makeNode('node-1', ['a', 'b']), makeNode('node-2', ['c', 'd'])] })
    let finishOld
    let failed = false
    api.world.fetchMapAtlasImage.mockImplementation(async (_project, id) => {
      if (id === 'b') return new Promise(resolve => { finishOld = resolve })
      if (id === 'd' && !failed) { failed = true; throw new Error('暂时不可用') }
      return new Blob([id])
    })
    const wrapper = mount(MapWorkspaceView, { props: { projectId: 'novel-1' } }); await flushPromises()
    await wrapper.get('.atlas-compare-toggle input').setValue(true); await flushPromises()
    await wrapper.findAll('.atlas-tree button')[1].trigger('click'); await flushPromises()
    finishOld(new Blob(['b'])); await flushPromises()
    expect(wrapper.get('.atlas-compare-toggle input').element.checked).toBe(false)
    expect(wrapper.findAll('.atlas-images img').map(item => item.attributes('alt'))).toEqual(['c 地图'])
    await wrapper.get('.atlas-compare-toggle input').setValue(true); await flushPromises()
    expect(wrapper.get('.atlas-image-state[role="alert"]').text()).toContain('图片读取失败')
    await wrapper.get('.atlas-image-state[role="alert"] button').trigger('click'); await flushPromises()
    expect(wrapper.findAll('.atlas-images img').map(item => item.attributes('alt'))).toEqual(['d 已采用地图', 'c 地图'])
    expect(wrapper.get('select[aria-label="选择左侧图片"]').findAll('option').map(item => item.attributes('value'))).toEqual(['d'])
  })

  it('同名图片选择项显示已知用途和日期，比较时不会拖动原标注', async () => {
    vi.stubGlobal('matchMedia', vi.fn(query => ({ matches: query === '(pointer: fine)', addEventListener: vi.fn(), removeEventListener: vi.fn() })))
    const pages = ['a', 'b', 'c'].map(id => page({ id, title: '同名地图', review_status: 'adopted', annotations: id === 'a' ? [{ id: 'annotation-a', label: '北门', position_x: 0.2, position_y: 0.2 }] : [] }))
    const atlas = tree(pages, 'atlas'); atlas.nodes[0].current_revision_id = 'saved-map'
    api.world.getMapAtlas.mockResolvedValue(atlas)
    ...
    expect(choices).toContain('同名地图 · 底图 · ')
    expect(choices).toContain('同名地图 · 水仙花街2号配图 · ')
    expect(choices).toContain('同名地图 · 普通图片 · ')
    expect(choices).toContain(new Date(pages[0].created_at).toLocaleString())
    await wrapper.get('.atlas-tree button').trigger('click'); await flushPromises()
    expect(wrapper.findAll('.atlas-images figure')).toHaveLength(2)
    await wrapper.get('.atlas-images figure:last-child .atlas-annotation').trigger('pointerdown', { clientX: 10, clientY: 10 })
    globalThis.dispatchEvent(new MouseEvent('pointermove', { clientX: 100, clientY: 100 }))
    globalThis.dispatchEvent(new MouseEvent('pointerup')); await flushPromises()
    expect(api.world.updateMapAtlasAnnotation).not.toHaveBeenCalled()
    expect(api.world.reviewMapAtlasPage).not.toHaveBeenCalled()
  })
