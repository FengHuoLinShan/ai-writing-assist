...
test.describe("世界书工作台", () => {
  ...
  test("页面创建、正文保存、投影刷新、审核弹窗和子视图切换都可用", async ({ page }) => {
    ...
    await expect(page.locator(SEL.modalTitle)).toHaveText("为页面新建分类")
    await page.locator("[data-bible-category-preset='technology']").click()
    const categoryRequest = page.waitForRequest((request) => (
      request.method() === "POST" && request.url().includes("/api/world/bible/categories")
    ))
    await page.getByRole("button", { name: "创建并用于此页面", exact: true }).click()
    const categoryPayload = (await categoryRequest).postDataJSON()
    expect(categoryPayload).toMatchObject({ category_key: "technology", name: "技术体系" })
    await expect(page.locator(SEL.modalTitle)).toHaveText("新建世界书页面")
    await expect(page.locator("#bible-create-title")).toHaveValue("E2E 世界基本背景")
    await expect(page.locator("#bible-create-template")).toHaveValue("e2e_trade_guide")
    await expect(page.locator("#bible-create-type")).toHaveValue("technology")
    const draftRequest = page.waitForRequest((request) => (
      request.method() === "POST" && request.url().endsWith("/api/world/bible/drafts")
    ))
    await page.locator(SEL.modalFooter).locator(SEL.btnPrimary).click()
    expect((await draftRequest).postDataJSON()).toMatchObject({ page_type: "technology" })
    ...
    const technologyCategory = categoryGroup.locator("[data-category='technology']")
    await expect(allCategory).toHaveAttribute("aria-pressed", "true")
    await expect(technologyCategory).toHaveAttribute("aria-pressed", "false")
    await technologyCategory.click()
    await expect(allCategory).toHaveAttribute("aria-pressed", "false")
    await expect(technologyCategory).toHaveAttribute("aria-pressed", "true")
    await expect(page.locator(".world-bible-page-card")).toContainText("E2E 世界基本背景")
    await page.locator(".world-bible-page-card", { hasText: "E2E 世界基本背景" })
      .locator("[data-action='bible-open-page-card']")
      .click()
    ...
    await expect(page.locator(SEL.modalBody)).not.toContainText("类别键")
  ...
  test("390px 下世界书表单、操作区和 AI 转交入口保持可用", async ({ page }) => {
    ...
    await expect(page.locator("#bible-create-type")).toHaveValue("culture_language")
    await page.locator(SEL.modalFooter).locator(SEL.btnPrimary).click()
    ...
    await expect(page.locator(SEL.modalTitle)).toHaveText("保存后打开 AI 工具")
    await page.getByRole("button", { name: "保存并继续" }).click()
    await expect(page.locator(SEL.toastContainer)).toContainText("工作稿已保存", { timeout: 10000 })
    await expect(page).toHaveURL(/\/world\/bible\?/)
