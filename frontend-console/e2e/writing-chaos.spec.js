...
test.describe("写作路径 chaos", () => {
  ...
  test("S4-STA-001 切换章节与 Scene 后只显示新 Scene 上下文", async ({ page }) => {
    ...
    await waitWritingReady(page, { chapter: 1 })
    await selectWritingChapter(page, 1)
    await expect(page.locator(".scene-tree-label--current")).toContainText(firstScene.title)
    await expect(page.locator("#writing-panel-container")).toContainText(firstScene.title)

    await selectWritingChapter(page, 2)
    const secondSceneLabel = page.locator(".scene-tree-label", { hasText: secondScene.title })
    await secondSceneLabel.click()

    await expect(page.locator(".scene-tree-label--current")).toContainText(secondScene.title)
    await expect(page.locator(".scene-tree-label--current")).not.toContainText(firstScene.title)
