...
test("对象详情明确授权补全，撤销需确认且发送同一任务", async ({ page, projectFactory }) => {
  ...
  await page.locator(".world-type-card").filter({ hasText: "地点" }).click()
  await page.locator(".world-bible-page-card").filter({ hasText: "北港" }).getByRole("button", { name: "打开", exact: true }).click()
