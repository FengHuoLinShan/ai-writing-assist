...
- `world/review` 把对象、别名和关系收在同一待定工作台：桌面端为队列 + 决策栏，390px 为队列/详情分步页。别名以“名称 / 称谓 / 身份”、关系以七类最小语义分类作为 AI 检索层，详细类型继续保留作者表达；普通列表和审核卡只显示中文分类与详细类型，缺分类统一显示“待分类”。普通别名和保守推荐关系可在决策栏直接采用；分别采用、换端点、归并和忽略收在次级编辑器。端点搜索调用项目对象接口，不受当前 20 条列表限制。
...
- 别名和关系编辑器采用“通用分类 + 详细类型”双字段：详细类型仅在分类为空时提供目录默认，不覆盖作者已选分类；未收录类型仍可作为自定义详细类型保留，映射建议必须点击才会修改。缺分类的别名直接/批量采用，以及关系采用、归并、分别采用都会在请求前阻断；忽略不受影响。批量采用/忽略均一次确认、一次请求，上限 50 条；部分失败或过期项保留选中和编辑草稿便于重试。由建议队列拥有的 compatibility shadow 别名不可在该工作台另行采用。
...
- 所有待处理世界对象都可在采用前微调名称、类型和概要；普通 candidate 通过 promote 请求在同一事务中编辑并采用，建议队列对象继续走队列裁决。待处理别名和关系以单项直接提交为主，会归档、忽略或复用已有资料的操作仍要求二次确认；未提交输入按项目和执行指纹保存在会话草稿中。
