...
- 作者入口会校验本地当前作品是否仍属于当前账户：有效时直接进入
  ...
  审核使用精确对象或组标识定位，不受默认 20 条分页影响。首页不改写作品；摘要失败不阻止进入写作，未知任务不会被静默清除。
...
- 结构、Scene 与世界书的可切换子导航使用原生按钮，当前项公开为当前页；Scene 工作台自身仍是非交互当前项，避免同路由刷新。查找不设置内容级 tab，索引健康、trace、重建、prewarm 与失败重试只从“索引诊断”次级入口进入，并在修复卡内返回查找。AI 地图册的候选、正式画廊与历史独立呈现，热点是可用键盘和触控打开的原生按钮；采用、拒绝与移出操作不会彼此替代。
...
- 写作台自动保存以编辑序号保护请求期间的新输入；版本切换触发局部重绘时不会用旧响应覆盖正文。发布、生成、冲突检查和自动提取的生命周期反馈统一从页面上缘下滑到写作区顶部居中浮层，不挤动正文，底部状态栏始终吸底；同一生命周期的成功/失败不再重复发送全局 toast。后台 RAG / 记忆后处理完成只收起进度，不再次重载当前编辑器，且已 dispose 或已被新任务取代的轮询响应不会回写。恢复历史正文时保留选择当时的最新版本快照，发布前若其他会话已更新则提示 409 冲突。写作台从设置服务读取项目生效作者偏好，日目标、编辑器字体和默认专注模式遵循“项目覆盖 → 全局默认 → 系统默认”，只在接口不可用时兼容读取旧本地偏好。“AI 工具”菜单与“专注模式”位于编辑器顶部同一操作行，正文 Scene 提取统一使用 imports 的“从正文提取 Scene”入口。顶部“AI 续写”锁定当前服务端 base draft，任务轮询不设前端总截止时间，完成后自动打开只读候选；候选统一提供“采用到工作稿”和“拒绝建议”，拒绝仅软废弃并保留审计。普通正文候选不会因 POV 检查明确不适用而显示角色视角解析失败。
...
- Scene 工作台单选直接执行当前主动作；混合选择按待办类型分组且一次处理一组，未处理组保持
  选中。结构类提醒可标记为无需整理并从更多菜单恢复；正文定位和融合建议仍独立处理。
...
- `world/review` 把对象、别名和关系收在同一待定工作台：桌面端为队列 + 决策栏，390px 为队列/详情分步页。别名以“名称 / 称谓 / 身份”、关系以七类最小语义分类作为 AI 检索层，详细类型继续保留作者表达；普通列表和审核卡用中文显示通用分类与目录内详细类型，未收录详细类型保留原文并标记“自定义”，缺分类统一显示“待分类”。普通别名和保守推荐关系可在决策栏直接采用；分别采用、换端点、归并和忽略收在次级编辑器。端点搜索调用项目对象接口，不受当前 20 条列表限制。已采用关系和别名也可在普通列表编辑双层类型或移动到其他有效对象。
...
- 待处理别名与关系的高级筛选控件均有明确的中文可访问名称，并分别支持 `alias_kind` / `relation_kind` 分类筛选；既有 `type_kind=recommended|custom`、详细类型、分页和 URL 草稿恢复语义保持兼容。Workflow ID 仍只在“诊断筛选”内以诊断字段暴露。
- 已采用对象库的多选工具条提供“融合”和“标记为别名”，不再提供“批量标记已检查”或“批量采用”。两种操作都要求选中至少两个同类型对象、明确选择保留对象并二次确认；融合会合并内容和关系，别名化只迁移关系和登记来源名称，来源对象均进入可审计历史态。
- 所有待处理世界对象都可在采用前微调名称、类型和概要；普通 candidate 通过 promote 请求在同一事务中编辑并采用，建议队列对象继续走队列裁决。待处理别名和关系以单项直接提交为主，会归档、忽略或复用已有资料的操作仍要求二次确认；未提交输入按项目和执行指纹保存在会话草稿中，指纹改变后不重放旧输入。
...
- 查找、Context 和 owner AI 抽屉不强制提供批量操作，只保留清晰的空状态、错误状态和任务进度；索引健康、trace、重建、prewarm 与失败重试收在“索引诊断”次级入口。
