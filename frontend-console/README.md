...
- `preset=world_core` 复用同一 Generate world 工作区，不新建工作台。空项目的 Writing Home 唯一主入口为“从几个灵感开始”；四个快捷动作只预填 `expand / connect / pressure / consolidate` 对应的可编辑作者消息，不自动发送。只计成功的作者—AI 回合；第三回合后仅提示保存，必须先由作者显式收束并通过 seed 覆盖、3–7 条规则、阻断矛盾和日常＋故障纵切门，才能手动保存 `world_core_checkpoint.v1`。checkpoint 只保存作者种子去向、规则、决定、来源 hash 与父版 lineage，不可采用；跨设备恢复只把该决定摘要作为后续参考，不重放过时的 AI 聊天正文。未显式保存时仍只保证当前浏览器恢复。
