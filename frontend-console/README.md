...
- 作者入口会校验本地当前作品是否仍属于当前账户：有效时直接进入
  ...
  审核使用精确对象或组标识定位，不受默认 20 条分页影响。Today 不在首页改写作品；摘要失败不阻止进入写作，未知任务不会被静默清除。
...
- 结构、Scene 与世界书的可切换子导航使用原生按钮，当前项公开为当前页；Scene 工作台自身仍是非交互当前项，避免同路由刷新。RAG search 不设置内容级 tab，status 仅由降级通知条或深链进入，并在修复卡内返回 search。AI 地图册的候选、正式画廊与历史独立呈现，热点是可用键盘和触控打开的原生按钮；采用、拒绝与移出操作不会彼此替代。
...
- 根路由 `home` 展示作者与“进入互动故事”双入口；RP 使用 `journeys` 列表与
  ...
  `today` 只组合现有项目摘要、世界书列表、建议列表和 task API，不发起生成；待决建议进入摘要投影后不再重复出现在“未完成创作”。它按现有 task API 逐项恢复该项目的持久化任务。`outline` 离开时停止 workflow 轮询并从持久化记录恢复，`writing` 使用项目隔离的显式
...
- `preset=world_core` 复用同一 Generate world 工作区，不新建工作台。空项目的 Today 唯一主入口为“从几个灵感开始”；四个快捷动作只预填 `expand / connect / pressure / consolidate` 对应的可编辑作者消息，不自动发送。只计成功的作者—AI 回合；第三回合后仅提示保存，必须先由作者显式收束并通过 seed 覆盖、3–7 条规则、阻断矛盾和日常＋故障纵切门，才能手动保存 `world_core_checkpoint.v1`。checkpoint 只保存作者种子去向、规则、决定、来源 hash 与父版 lineage，不可采用；跨设备恢复只把该决定摘要作为后续参考，不重放过时的 AI 聊天正文。未显式保存时仍只保证当前浏览器恢复。
- Today 对 pending `world_adoption_package.v1` 优先显示“去审查”，进入 World Bible 后分栏预览“流水线已写入”与“本次确认将写入”。前端只提交 preview hash；409 要求重新预览，不显示 raw ID/JSON，不绕过后端的 DB + World Bible 原子 apply。
...
- 作者动作标签不形成新队列：Today 读取现有领域队列的只读投影；写作字面预警和普通软判断标为“可以改进”，需要人工选择的 World/Outline 项标为“需要决定”。已处理、忽略和稍后项退出当前投影；冲突详情显示来源版本与当前章／场景的定向复检范围，并把来源缺口写成作者选择而非自动阻断。workspace-summary 与 Writing conflict wire 仅增加可选字段，数据库 schema 不变。
