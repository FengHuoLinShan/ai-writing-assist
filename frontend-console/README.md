...
- 作者动作标签不形成新队列：Writing Home 读取现有领域队列的只读投影；写作字面预警和普通软判断标为“可以改进”，需要人工选择的 World/Outline 项标为“需要决定”。已处理、忽略和稍后项退出当前投影；冲突详情显示来源版本与当前章／场景的定向复检范围，并把来源缺口写成作者选择而非自动阻断。workspace-summary 与 Writing conflict wire 仅增加可选字段，数据库 schema 不变。
