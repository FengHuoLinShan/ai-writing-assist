  ...
  <div v-else class="generate-pov-workspace">
    <div class="card generate-pov-form">
      ...
      <div class="generate-form-grid">
        ...
        <label>场景 *<select id="generate-pov-scene" :value="form.sceneId" class="form-select" :disabled="!form.chapterIndex" @change="$emit('change-scene', $event.target.value)"><option value="">请选择场景</option><option v-for="scene in scenes" :key="scene.id" :value="scene.id">{{ scene.title || scene.name || '未命名场景' }}</option></select></label>
      ...
      <div v-if="manualRole" class="generate-pov-note">本次使用手动选择角色，不修改场景的视角设置。</div>
      <div v-if="sceneWithoutPov" class="generate-pov-note">当前场景未设置视角角色，请手动选择本次生成角色。</div>
