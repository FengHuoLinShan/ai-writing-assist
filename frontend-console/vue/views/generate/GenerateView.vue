...
function openConvergenceSource(source) {
  ...
  if (type === "map_fact") return router.navigate("world", "map", true)
...
async function applyWorldPage(payload) {
  ...
  try { const response = await api.generate.applyWorldPageDraft(suggestionId, payload, props.projectId, { signal: scope.controller.signal }); if (!owner.isActive(scope)) return; discardPageProposalDraft(); persist(); writeCreativeContinuation(props.projectId, { destination: "world_bible_draft", route: { draft_id: response.draft.id, page_id: response.draft.page_id || null } }); toast("提案已应用到工作稿，尚未发布", "success"); const query = new URLSearchParams({ draft_id: response.draft.id }); if (response.draft.page_id) query.set("page_id", response.draft.page_id); router.navigate("world", "bible", true, query) }
...
onBeforeUnmount(() => { disarmBeforeUnload(); persist(); owner.dispose(); visualQuickCreate.close(); if (ownedModal) closeModal() })
</script>

<style>
.generate-visual-brief{flex:0 0 auto;margin:0 0 10px;border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px 10px;color:var(--text-muted);font-size:12px}.generate-visual-brief>summary{cursor:pointer;color:var(--text);font-weight:600}.generate-visual-brief__axes{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:8px 0}.generate-visual-brief__axes span{display:grid;gap:2px;border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px}.generate-visual-brief__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:8px}.generate-visual-brief__grid label{display:grid;gap:4px}.generate-visual-brief__wide{grid-column:1/-1}
.generate-external-handoff{flex:0 0 auto;margin:0 0 10px;border-top:1px solid var(--border);padding-top:8px;color:var(--text-muted);font-size:12px}.generate-external-handoff>summary{cursor:pointer;color:var(--text)}.generate-external-handoff__input{display:grid;gap:5px;margin-top:8px}.generate-external-handoff__meta{display:grid;gap:2px;margin-top:5px;color:var(--text-dim)}.generate-external-handoff__meta.is-over-limit{color:var(--warning)}.generate-external-packet-history{display:grid;gap:6px;margin-top:10px}.generate-external-packet-history article{display:grid;gap:2px;border-top:1px solid var(--border);padding-top:6px}.generate-external-packet-history code{overflow-wrap:anywhere;color:var(--text-dim);font-size:10px}.generate-handoff-manual{width:100%;font:12px/1.5 var(--font-mono)}
.generate-exploration{flex:0 0 auto;margin:0 0 10px;border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px 10px;color:var(--text-muted);font-size:12px}.generate-exploration>summary{cursor:pointer;color:var(--text);font-weight:600}.generate-exploration__results{display:grid;gap:8px;margin-top:8px}.generate-exploration .generate-convergence-card.is-selected{border-color:var(--accent);background:var(--selected)}
.topbar-generate-note{margin-left:10px;color:var(--text-secondary);font-size:12px;font-style:italic;white-space:nowrap}.generate-suggestion-note{max-width:260px;color:var(--text-dim);font-size:11px;line-height:1.35}.generate-tab-panel{min-width:0}.generate-chatbox{display:grid;grid-template-columns:minmax(0,78fr) minmax(180px,22fr);gap:12px;align-items:stretch;height:calc(100vh - 180px);min-height:480px;overflow:hidden}.generate-chatbox:has(.generate-side-rail:not([open])){grid-template-columns:minmax(0,1fr) var(--workspace-rail-collapsed)}.generate-chat-main{min-height:0;overflow:hidden}.generate-chat-panel{display:flex;flex-direction:column;height:100%;min-height:0;overflow:hidden}.generate-chat-side{min-height:0;max-height:100%;overflow:auto;padding-right:2px}.generate-settings-card{margin-bottom:12px}.generate-card-title-row,.generate-side-options,.generate-template-row,.generate-result-actions,.generate-world-targets{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.generate-card-title-row{justify-content:space-between;margin-bottom:10px}.generate-quality-toggle{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text-muted)}.generate-chat-messages{flex:1 1 auto;min-height:0;overflow:auto;border:1px solid var(--border);border-radius:var(--radius-md);padding:18px;background:var(--bg);margin-bottom:12px}.generate-chat-message{margin-bottom:10px;max-width:92%}.generate-chat-message.assistant{margin-left:auto}.generate-chat-role,.generate-result-meta,.generate-empty-copy{color:var(--text-dim);font-size:12px}.generate-chat-bubble{white-space:pre-wrap;border:1px solid var(--border);border-radius:var(--radius-md);padding:10px 12px;background:var(--panel);font-size:13px;line-height:1.55}.generate-chat-message.error .generate-chat-bubble,.generate-error-text{color:var(--danger)}.generate-convergence-action{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:-2px 0 10px;color:var(--text-muted);font-size:12px}.generate-convergence-action strong,.generate-convergence-action span{display:block}.generate-convergence{flex:0 1 48vh;min-height:0;overflow:auto;margin-bottom:10px;border:1px solid var(--accent);border-radius:var(--radius-md);padding:12px;background:var(--panel)}.generate-convergence__header,.generate-convergence-card__title,.generate-convergence-actions,.generate-convergence-sources,.generate-convergence__stats{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.generate-convergence__header,.generate-convergence-card__title{justify-content:space-between}.generate-convergence__header p,.generate-convergence-card p{margin:3px 0;color:var(--text-muted);font-size:12px}.generate-convergence__stats{margin:8px 0;color:var(--text-dim);font-size:11px}.generate-convergence-card{margin-top:10px;border:1px solid var(--border);border-radius:var(--radius-sm);padding:10px}.generate-convergence-card__title span,.generate-convergence-sources>span{color:var(--text-dim);font-size:11px}.generate-convergence-list{margin:6px 0;padding-left:20px}.generate-convergence-items{display:grid;gap:7px;margin-top:8px}.generate-convergence-items label{display:grid;grid-template-columns:minmax(0,1fr) minmax(132px,180px);align-items:center;gap:8px}.generate-convergence-sources{margin-top:8px}.generate-convergence-boundary{border-left:3px solid var(--accent);padding-left:8px}.generate-convergence-message{display:grid;gap:5px;font-size:12px}.generate-convergence-message textarea{min-height:150px}.generate-convergence-actions{margin-top:8px}.generate-composer{position:relative;display:flex;align-items:flex-end;flex:0 0 auto;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--panel);padding:8px}.generate-chat-input{width:100%;min-height:72px;padding-right:76px;resize:vertical;border:0;outline:0;background:transparent;color:var(--text);font:inherit}.generate-composer-send{position:absolute;right:10px;bottom:10px;min-width:58px}.generate-template-row--toolbar{margin-bottom:var(--space-2)}.generate-template-btn,.generate-world-target,.generate-subtab{border:1px solid var(--border);background:var(--panel);color:var(--text);border-radius:var(--radius-sm);padding:6px 10px;cursor:pointer}.generate-template-btn.active,.generate-world-target.active,.generate-subtab.active{border-color:var(--accent);background:var(--selected);color:var(--accent)}.generate-result-card{border:1px solid var(--accent);border-radius:var(--radius-sm);padding:12px;background:var(--panel)}.generate-result-title{font-weight:600;margin-bottom:6px}.generate-result-actions{margin-top:10px}.generate-world-source-bar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;padding:9px 12px}.generate-world-source-label{color:var(--text-dim);font-size:12px;margin-right:8px}.generate-world-config{display:flex;align-items:end;gap:10px;padding:8px 10px;margin-bottom:8px;border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-muted);font-size:12px}.generate-world-context-panel{display:grid;gap:8px;margin-top:10px;border-top:1px solid var(--border);padding-top:8px}.generate-world-context-panel label,.generate-page-result label,.generate-pov-form label{display:grid;gap:4px;color:var(--text-muted);font-size:12px;margin-top:7px}.generate-page-result{display:grid;gap:8px}.generate-json-editor{font:11px/1.45 var(--font-mono)}.generate-task-workspace{display:grid;grid-template-columns:minmax(190px,22fr) minmax(0,78fr);gap:12px;align-items:start}.generate-task-cards{display:grid;gap:8px}.generate-task-card{border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px 10px;cursor:pointer;text-align:left;background:var(--panel);color:var(--text)}.generate-task-card.active{border-color:var(--accent);background:var(--selected)}.generate-task-card h4{margin:0 0 2px}.generate-task-card p{margin:0;color:var(--text-dim);font-size:12px}.generate-task-form .form-group{margin-bottom:10px}.generate-pov-workspace{display:grid;grid-template-columns:minmax(0,72fr) minmax(200px,28fr);gap:12px}.generate-pov-form{display:grid;gap:10px}.generate-form-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.generate-template-warning,.generate-pov-note{border:1px solid var(--warning);border-radius:var(--radius-sm);color:var(--warning);padding:8px;font-size:12px}.generate-chapter-list{display:grid;gap:8px;max-height:460px;overflow:auto}.generate-chapter-card{display:grid;grid-template-columns:auto minmax(0,1fr);gap:8px;border:1px solid var(--border);padding:8px}.generate-chapter-title{display:block;font-weight:600}.generate-chapter-excerpt{display:block;color:var(--text-dim);font-size:12px}.generate-template-editor{display:grid;gap:10px}.generate-template-editor label{display:block}.generate-template-editor textarea{min-height:180px}.generate-template-history{display:grid;gap:8px;max-height:320px;overflow:auto}.generate-template-revision{display:grid;gap:6px;border:1px solid var(--border);padding:8px}.generate-template-revision pre{white-space:pre-wrap;max-height:120px;overflow:auto}.generate-context-header,.generate-context-tags{display:flex;gap:8px;flex-wrap:wrap}.generate-context-meta,.generate-context-hint{color:var(--text-muted);font-size:12px}.generate-context-tag{border:1px solid var(--border);padding:2px 5px}.generate-markdown-pre{white-space:pre-wrap}.generate-pov-summary{display:grid;gap:5px;margin-top:12px;color:var(--text-dim);font-size:12px}@media(max-width:900px){.generate-chatbox,.generate-chatbox:has(.generate-side-rail:not([open])),.generate-task-workspace,.generate-pov-workspace,.generate-form-grid,.generate-visual-brief__axes,.generate-visual-brief__grid{grid-template-columns:1fr;height:auto;min-height:0;overflow:visible}.generate-visual-brief__wide{grid-column:auto}.generate-side-rail{grid-column:1/-1}.topbar-generate-note,.generate-suggestion-note{display:none}.generate-world-source-bar,.generate-world-config,.generate-convergence-action{align-items:stretch;flex-direction:column}.generate-convergence{max-height:none}.generate-convergence-items label{grid-template-columns:1fr}}@media(max-width:390px){.generate-visual-brief .btn{min-height:44px}}
@media(max-width:390px){.generate-exploration .btn{min-height:44px}.generate-convergence-actions .btn{flex:1 1 100%;min-height:44px}}
</style>
