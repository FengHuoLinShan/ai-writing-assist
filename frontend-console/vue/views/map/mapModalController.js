...
export function createMapModalController({
  ...
  const ownsControl = (token, control, id) => owns(token)
    && control?.isConnected
  ...
  function showCreateWorld() {
    ...
    `, [{
      text: "创建", class: "btn-primary", handler: async () => {
        const nameControl = document.getElementById("map-create-name")
        const ownsModal = () => ownsControl(token, nameControl, "map-create-name")
        ...
        try {
          ...
          await onCreated?.(created, ownsModal)
          return true
        } catch (error) {
          if (!ownsModal()) return true
          toast(`创建失败：${error.message || "未知错误"}`, "error")
          return false
        }
      },
    }])
  ...
  function showAssign(item) {
    ...
    showModalHtml("分配地图待处理项", `<p>分配后将打开目标地图，并继续补全「${esc(item.target_name || item.dynamic_type || "地图建议")}」。</p><div class="form-group"><label>目标地图</label><select class="form-select" id="map-inbox-assignment-map">${maps.map((map) => `<option value="${esc(map.id)}" ${map.id === (item.map_id || getActiveMapId?.()) ? "selected" : ""}>${esc(map.name)}</option>`).join("")}</select></div>`, [{
      text: "分配并继续", class: "btn-primary", handler: async () => {
        const mapControl = document.getElementById("map-inbox-assignment-map")
        const ownsModal = () => ownsControl(token, mapControl, "map-inbox-assignment-map")
        if (!ownsModal()) return true
        const mapId = mapControl?.value
        if (!mapId) return false
        const success = await onAssigned?.(item, mapId, ownsModal)
        return ownsModal() ? success : true
      },
    }])
    return true
  }
  ...
  function showRestore(mapId) {
    ...
    showModalHtml("恢复归档地图", `<p>将恢复「${esc(map.name)}」及其完整子树。</p><div class="form-group"><label>恢复根名称</label><input class="form-input" id="map-restore-root-name" maxlength="255" value="${esc(map.name)}" /></div>`, [{
      text: "恢复子树", class: "btn-primary", handler: async () => {
        const nameControl = document.getElementById("map-restore-root-name")
        const ownsModal = () => ownsControl(token, nameControl, "map-restore-root-name")
        if (!ownsModal()) return true
        const rootName = nameControl?.value?.trim() || map.name
        const success = await onRestored?.(map, rootName, ownsModal)
        return ownsModal() ? success : true
      },
    }])
    return true
  ...
  function showDynamicItem(item) {
    ...
    if (item.item_kind === "observation") buttons.push(
      ...
      { text: "标记冲突", class: "", handler: () => onConflictObservation?.(item) },
      { text: "更换地图", class: "", handler: () => showAssign(item) },
      { text: "取消分配", class: "", handler: () => onUnassignObservation?.(item) },
    )
    if (item.item_kind === "fact") buttons.push(
      { text: "回滚", class: "", handler: () => onFactStatus?.(item, "rolled_back") },
      { text: "废弃", class: "", handler: () => onFactStatus?.(item, "deprecated") },
      { text: "恢复采用", class: "btn-primary", handler: () => onFactStatus?.(item, "confirmed") },
    )
    buttons.push(
      { text: "复制诊断信息", class: "", handler: () => copyDiagnostic(item) },
      { text: "打开检查器", class: "", handler: () => onFocusInspector?.(item) },
    )
