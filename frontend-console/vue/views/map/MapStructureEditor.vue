<template>
  <dialog ref="leaveDialog" class="map-leave-dialog" aria-labelledby="map-leave-title" @cancel.prevent="finishLeave(false)" @close="finishLeave(false)">
    <h2 id="map-leave-title">地图有未保存修改</h2>
    <p>保存到作品，或保留当前浏览器中的草稿后离开。</p>
    <p v-if="error" role="alert">{{ error }}</p>
    <div class="map-actions">
      <button class="btn btn-primary" :disabled="saving" @click="saveAndLeave">{{ saving ? '正在保存…' : '保存并离开' }}</button>
      <button class="btn" :disabled="saving || (!backedUp && !downloaded)" @click="finishLeave(true)">{{ backedUp ? '保留草稿离开' : '确认已保留下载备份并离开' }}</button>
      <button class="btn" :disabled="saving" @click="finishLeave(false)">继续编辑</button>
    </div>
  </dialog>
  <section ref="toolsRoot" class="map-editor" :class="{ 'map-focused': focused }" aria-label="空间地图编辑器">
    <header v-if="!externalTools || dirty || focused" class="map-toolbar">
      ...
      <div class="map-actions">
        ...
        <button v-if="images.length && !externalTools && !readOnly && !focused" class="btn btn-sm" @click="chooseImageRole('background')">设置底图</button>
    ...
    <template v-if="!referenceOnly">
      ...
      <aside v-if="inspectorOpen && selectedFeature && !readOnly && !focused" class="map-inspector" aria-label="地图内容详情">
        ...
        <label v-if="['location', 'landmark', 'area'].includes(selectedFeature.kind)">关联世界地点<select :value="selectedFeature.entity_id || ''" class="form-select" @change="bindEntity($event.target.value)"><option value="">独立地图标记</option><option v-if="selectedFeature.entity_id && !bindingLocations.some(item => item.id === selectedFeature.entity_id)" :value="selectedFeature.entity_id">{{ boundName || selectedFeature.label }}（当前关联）</option><option v-for="location in bindingLocations" :key="location.id" :value="location.id">{{ location.name }}</option></select></label>
        <MapBindAcrossMaps :project-id="projectId" :node-id="node.id" :entity-id="selectedFeature.entity_id" :entity-name="boundName || selectedFeature.label" :nodes="knownNodes" :disabled="dirty || busy" />
        <section class="map-feature-sources" aria-label="地点资料依据">
          ...
          <article v-for="(source, index) in selectedFeature.sources" :key="index">
            ...
            <div class="map-actions"><button v-if="source.kind === 'source_range'" type="button" class="btn btn-sm" @click="openSourcePicker(source)">预览原文</button><button v-if="source.kind === 'source_range' && source.source_ref?.chapter_index" class="btn btn-sm" @click="openSourceChapter(source)">进入第 {{ source.source_ref.chapter_index }} 章编辑</button><button type="button" class="btn btn-sm" :disabled="busy" :aria-label="'移出第 ' + (index + 1) + ' 条依据'" @click="removeSource(index)">移出依据</button></div>
          </article>
        </section>
    ...
    <details ref="imageControls" v-if="!readOnly && !focused && images.length" class="map-image-controls">
...
const api = getApi(), confirm = message => confirmAsync(message, "确认", { confirmAction: getConfirmAction() })
...
const bindingQuery = ref(''), bindingLocations = ref([]), bindingLoading = ref(false), bindingError = ref(''), boundName = ref(''), imageControls = ref(null)
let bindingEpoch = 0
function chooseImageRole(role) {
  imageForm.role = role; imageForm.feature_id = role === 'illustration' ? selectedId.value : ''
  if (imageControls.value) { imageControls.value.open = true; imageControls.value.scrollIntoView({ block: 'nearest' }) }
}
async function searchBindings() {
  const token = ++bindingEpoch
  bindingLoading.value = true; bindingError.value = ''
  try {
    const result = await api.world.listEntities({ novel_id: props.projectId, entity_type: 'location', status: 'canonical', q: bindingQuery.value || undefined, limit: 20 })
    if (!alive || token !== bindingEpoch) return
    bindingLocations.value = (Array.isArray(result) ? result : result.items || []).filter(item => item.status === 'canonical')
    const id = selectedFeature.value?.entity_id
    if (id) {
      const entity = bindingLocations.value.find(item => item.id === id) || await api.world.getEntity(id, props.projectId)
      if (alive && token === bindingEpoch) boundName.value = entity.name
    }
  } catch (err) { if (alive && token === bindingEpoch) bindingError.value = err.message || '地点读取失败，请重试' }
  finally { if (alive && token === bindingEpoch) bindingLoading.value = false }
}
...
let epoch = 0, alive = true, installing = false, backupTimer = null, pollTimer = null, drag = null, reviewEpoch = 0, imageBaselineEpoch = 0, comparisonEpoch = 0, lastLoadError = '', pan = null
...
async function removeFeature() {
  const feature = selectedFeature.value
  if (!feature || !await confirm("从此地图移出“" + feature.label + "”及相关约束？关联图形需要重新定位，世界资料仍会保留。")) return
...
async function review(value, action, changeKeys = null) {
  ...
  if (action === "restore" && !await confirm("将历史地图恢复为新版本？当前版本仍保留在历史中。")) return
  if (action === "reject" && !await confirm("不使用这个空间候选？已保存地图不会受到影响。")) return
  saving.value = true
  try {
...
async function rebaseManually() {
  if (recovery.value) return
  const before = JSON.stringify(doc.value), server = serverRevision.value
  if (!await confirm("确认已比较服务器版，并用当前编辑创建一个新版？")) return
  if (JSON.stringify(doc.value) !== before || serverRevision.value?.id !== server?.id) { reviewNotice.value = '编辑或服务器版本已变化，请重新核对。'; return }
  revision.value = server; conflict.value = false; compareServer.value = false; persistDraft()
}
...
function downloadBackup() {
  ...
  link.href = url; link.download = "地图编辑备份.json"; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); downloaded.value = true
}
const downloaded = ref(false)
const leaveDialog = ref(null)
let leaveDecision = null, settleLeave = null
function finishLeave(allowed) {
  const resolve = settleLeave
  settleLeave = null; leaveDecision = null
  leaveDialog.value?.close()
  resolve?.(allowed)
}
async function saveAndLeave() {
  await save()
  if (!dirty.value && !saving.value) finishLeave(true)
}
function canLeave() {
  if (saving.value || drag) return false
  if (!dirty.value) return true
  persistDraft()
  if (!backedUp.value && !downloaded.value) { error.value = "当前编辑还未保存或备份，请先保存或下载备份。"; return false }
  if (leaveDecision) return leaveDecision
  leaveDecision = new Promise(resolve => { settleLeave = resolve })
...
watch(doc, () => {
  ...
  backedUp.value = false; downloaded.value = false; clearTimeout(backupTimer); backupTimer = setTimeout(persistDraft, 200)
...
watch(selectedId, () => { bindingQuery.value = ''; boundName.value = ''; void searchBindings() })
...
onBeforeUnmount(() => { finishLeave(false); rememberView(); resizeObserver?.disconnect(); persistDraft(); alive = false; epoch += 1; clearTimeout(backupTimer); clearTimeout(pollTimer); globalThis.removeEventListener("beforeunload", beforeUnload); for (const url of Object.values(imageUrls)) URL.revokeObjectURL(url) })
