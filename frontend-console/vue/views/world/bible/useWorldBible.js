...
export function useWorldBible(props) {
  ...
  async function publishDraft() {
    ...
    const draft = savedDraftSnapshot.value
  ...
  // ---- suggestions ----
  async function openSuggestions(focusSuggestionId = "") {
    try {
      const query = {
        novel_id: projectId.value,
        source_module: "world",
      ...
      if (!disposed) toast(err.message || "加载建议失败", "error")
    }
  }
  ...
  async function applyEditedSuggestion(item) {
    ...
    try {
      ...
      if (draft?.id) {
        rememberDraft(draft)
        const query = new URLSearchParams({ draft_id: draft.id })
        if (draft.page_id) query.set("page_id", draft.page_id)
  ...
  async function inspectCurrentPage() {
    ...
    const draft = (
      savedDraftSnapshot.value?.page_id === page.id
        ? savedDraftSnapshot.value
