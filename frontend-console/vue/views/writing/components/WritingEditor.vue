  ...
  <div class="writing-editor-shell">
      ...
      <div id="writing-editor-buttons" class="writing-editor-buttons">
        ...
        <details v-if="hasChapters" class="writing-tools-menu">
          ...
          <div class="writing-tools-menu__body">
            ...
            <div class="writing-tools-menu__group">
              <strong>工具</strong>
              <button class="btn btn-sm" :disabled="!hasChapter || state.readonly" @click="$emit('split-scene')">断章至此</button>
    ...
    <template v-else>
      ...
      <div id="writing-wordcount-bar" class="writing-wordcount-bar">
        ...
        <span v-if="dailyGoal">日目标 {{ state.content.length.toLocaleString() }} / {{ Number(dailyGoal).toLocaleString() }}</span>
      </div>
    </template>
  </div>
</template>
...
defineEmits([
  ...
  "generate-draft", "generate-continuation", "generate-pov", "split-scene",
