  ...
  if (subView === "objects" || props.bibleDeepLink.openObjectTools) {
    ...
    const detailPromise = (async () => {
      if (props.bibleDeepLink.draftId) {
        let draft
        try { draft = await api.world.getBibleDraft(props.bibleDeepLink.draftId, projectId) }
        catch (error) {
          if (Number(error?.status) !== 404 || !api.world.getBibleDraftPublication) throw error
          const published = await api.world.getBibleDraftPublication(props.bibleDeepLink.draftId, projectId)
          const page = await api.world.getBiblePage(published.page_id, projectId)
    ...
    props.bible = {
      ...
      activationProfiles: [],
      lazySupport: true,
      validationRun: query.get("validation_run_id") && api.world.getWorldValidationRun ? await api.world.getWorldValidationRun(query.get("validation_run_id"), projectId) : null,
