  ...
  _renderCompileResult(data) {
    ...
    if (data.budgets && data.budgets.length > 0) {
      ...
      for (const b of data.budgets) {
        ...
        html += `<tr><td style="color:var(--text-muted);">${esc(this._budgetName(b.category))}</td><td>${b.budget}</td><td style="color:${pctColor};">${b.used} (${pct}%)</td></tr>`
      }
      html += '</tbody></table>'
    }
    if (data.sections_present && data.sections_present.length > 0) {
      html += '<div style="margin-bottom:12px;"><strong style="color:var(--text-muted);font-size:12px;">加载的段落来源：</strong>'
      html += '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px;">'
      const sourceNames = { project: "📁 项目信息", world_entities: "🌍 世界对象", characters: "👤 人物档案", geo_locations: "🗺️ 地理地点", memory_records: "📝 长期记忆", timeline_events: "📅 时间线事件", plot_threads: "🧵 剧情线", outline_arc: "📖 篇章纲", chapter_card: "📄 章节卡", rag_chunks: "🔍 RAG 片段" }
      for (const section of data.sections_present) {
        html += `<span style="background:var(--panel);color:var(--text);padding:2px 8px;border-radius:3px;font-size:11px;border:1px solid var(--border);">${esc(sourceNames[section] || section)}</span>`
      }
      html += '</div></div>'
    }
