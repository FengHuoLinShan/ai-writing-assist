...
| 当前做到哪一步？ | 当前仓库具备双入口、作者导入/写作/世界设定/大纲/检索主链、基于确认资料的空间示意与可选图片统一地图，以及可选作品导入/复用、版本化 Context、对象引用、分支、流式恢复、自动回顾和看海循环；仍是工程验证系统。 |
...
flowchart TB
    ...
    subgraph Creative["创作系统：所有业务读写显式过滤 novel_id"]
        subgraph Facts["事实层"]
            ...
            World["world<br/>人物、地点、关系、时间线、统一地图"]
...
| `world` | 人物、地点、关系、时间线、事件等长期世界事实，以及来源可追溯、可编辑恢复且不反写正史的统一地图。 |
