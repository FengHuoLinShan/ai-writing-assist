...
## 1. 迁移（本轮声明 root_capability_id + L0=A + deadline）

| 能力（root） | task type | A | L0 | H 建议 | deadline | 备注 |
|---|---|---|---|---|---|---|
| writing.generate | writing_generate | 6⌈K/64⌉+8（resolver 从冻结 receipt 算 K） | =A | H≥A：K 由作者确认的 scope receipt 冻结，单次授权已隐含 | 1800s | W3-A |
| writing.semantic_review | writing_semantic_review | 144 | =A | 同上（chunk≤24 常量） | 1800s | W3-A |
| writing.targeted_revision | writing_targeted_revision | 4 | =A | — | 1800s | W3-A；错绑修正 |
| writing.conflict_check.ai_review / ai_suggestion | 对应两个 task | 6 / 6 | =A | — | 代码现有边界 | W3-A |
| story.story_outline.generate | story_outline_generate | 64 | =A | — | 1800s | W3-A |
| story.outline.analyze / p20 | outline_analyze / outline_generate | 2 / 64 | =A | — | — | W3-A |
| story.scene_fusion / character_card / reaction / script / one_click | 对应 task | 12 / 28 / 672 / 28 / 1372 | =A | reaction 的 672 由 schema max_length=24 冻结 | — | W3-A；one_click 为子链 canonical parent，子 step 归属 root |
...
| world.alias_relations.extract | world_alias_relation_extraction | 4S | =A | S 为作者所选章节范围 | — | W3-B |
| world.world_bible.synopsis | world_bible_synopsis_refresh | 36 | =A | — | 1800s | W3-B |
| world.generation.suggestion | world_generation_suggestion | 96（task 路径） | =A | — | 1800s | W3-B |
| world.generation.chat | world_cocreation_turn | 按代码实测（W3-B 报告） | =A | — | 1800s | W3-B |
...
| interaction.story_generate（agent） | interaction_agent_story_generate | 29（按代码实测修正） | =A | — | 1800s | W3-C |
| interaction.summary_refresh | interaction_summary_refresh | 8 | =A | — | 现有边界 | W3-C；错绑修正 |
| interaction.continuity_review | interaction_continuity_review | 9 | =A | — | provider 180s | W3-C |
| infrastructure.rag_query_planner | evidence_focused_search | 9（max_depth∈{0,1}×U(2,0)） | =A | max_depth 由请求 schema 冻结 | 600s | 主 Agent（已声明） |
