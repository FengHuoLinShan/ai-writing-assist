...
> 3. **`--topbar-height` 双源漂移**：壳层实际 68px/≤760 60px vs 变量 64px。修复：`.creative-shell` 定义 68px（≤760 60px），`#topbar` 尺寸引变量，全部消费方（outline 浮层、owner-ai 抽屉、场景抽屉、移动 toast、writing-desk 偏移）对齐。
...
> 5. **RP 定位轨道底边硬编码**：`.rp-locator-rail` bottom:150px 在多行输入（dock 可至 ~300px、z18>z16）时被盖。修复：`bottom:clamp(150px, 30dvh, 300px)`。
> 6. **RP 确认弹窗 maxHeight 无下界**（一.五 轮遗留理论风险）：anchor 贴边时 availableHeight 趋 0 弹窗压扁。修复：`calculateAdaptivePopoverPlacement` 增加 160px 可交互下限（表面已有内部滚动）；顺带把 `.rp-more-menu>div` 的 `--topbar-height` 错位引用改为基于故事页自身 58px 顶栏的 `calc(100dvh - 82px)`。
>
...
>
> **验证（2026-09-18）**：前端 ESLint、Vitest 193 文件/2503 项、生产构建、e2e 冒烟 57 项 + interaction 14 项（含 390px 窄屏故事页）+ workflow-toolbars 3 项全部通过；本地 1440/500×844 浏览器实测菜单层叠/钳位、journeys 导航闭环、68/60px 变量解析；`make docs-check BASE_REF=origin/main` 通过（同步 00_整体设计、14_frontend、frontend-console/README）；`git diff --check` 干净。线上复核需按 `deploy/scripts/release.sh` 发版后进行。
