...
- 候选没有实质覆盖 AUTHOR_BRIEF 要求的计划尺度和范围，只是把早期阶段
  扩写得更长，或用章数填充伪装长期设计。
- 作者要求覆盖高潮选择与结局，候选却只说“最终会选择并付代价”，把具体结果全部
  推回 open_decisions，未提供一个可审阅的主方案；作者明确保留开放时除外。
- 候选自身的金额、数量、时间或因果互相矛盾，导致主方案声称的解决方式不能成立；
  须核对各处同一笔账和资源总量，不得因是候选就放过明确的计算错误。

提出具体的候选结局不等于替作者采用。不能仅因作者没指定结局答案而要求删去主方案。
