"""G2 整体纵切（V4 计划 §9 最小纵切 / T03 / T07 / T17）。

虚构测试小说：林舟、青竹、白石城、铜钥匙。

    原文（Scene 0/1/2） → 前序状态 → 后序理解 → 新 case 消费
    → 地图在场（未知路线不造真） → 修订失效（旧建议失去有效资格）

每一步都经过真实内核：管线组合器（屏障/预算/采样/观察/身份/窄提交）、
E04 关系存储、E03c 回执纪律、E05 失效传播、在场投影与建议有效性缝。
"""
...
from modules.evolution.pipeline import BarrierBlockedError, run_scene_step
...
class _DeterministicSampler:
    """扮演 provider：按 Scene 返回固定观察与场景事件（不调用真实模型）。"""
                ...
                "observations": [
                    {
                        ...
                        "quote": "青竹从袖中取出铜钥匙",
                        "mentions": [
                            {"mention_id": "m-qingzhu", "surface": "青竹"},
                            {"mention_id": "m-linzhou", "surface": "林舟"},
                        ],
                    }
                ],
            }
                ...
                "observations": [
                    {
                        ...
                        "quote": "她要先回白石城东市的当铺",
                        "mentions": [{"mention_id": "m-qingzhu-2", "surface": "青竹"}],
                    }
                ],
            }
        return {
            ...
            "observations": [
                {
                    ...
                    "modality": "event_observed",
                    "quote": "没有说明自己如何离开白石城",
                    "mentions": [{"mention_id": "m-linzhou-3", "surface": "林舟"}],
                }
            ],
        }
...
async def test_g2_vertical_slice(
    ...
    # ---- 2. 后序理解：Scene 1 输入实际包含 Scene 0 已提交回执（T07） ----
