...
from modules.evidence.indexing import tasks as rag_tasks  # noqa: F401 — 注册任务处理器
...
async def test_retry_embeddings_endpoint_enqueues_task() -> None:
    ...
    from modules.evidence.indexing.api import retry_embeddings
    from modules.evidence.indexing.schemas import RagRetryEmbeddingsRequest
    ...
    with (
        patch(
            "modules.evidence.indexing.api._require_active_project",
            autospec=True,
        ) as guard,
        patch(
            "modules.evidence.indexing.api.enqueue_task",
            autospec=True,
