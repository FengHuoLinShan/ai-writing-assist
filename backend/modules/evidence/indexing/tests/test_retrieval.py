...
from modules.evidence.indexing.query_expansion import QueryExpander
...
from modules.evidence.indexing.retrieval import RetrievalOrchestrator, _is_rerank_enabled
from modules.evidence.indexing.scoring import Scorer
...
class TestRetrievalOrchestratorDedup:
    ...
    def test_deduplicate_limits_embedding_comparison_window(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "modules.evidence.indexing.scoring.cosine_similarity", _count_similarity
        )
        chunks = [
            type(
                "Chunk",
...
class TestRetrievalOrchestratorInjected:
    ...
    async def test_reranker_drops_only_low_value_topical_noise(
        ...
        monkeypatch.setattr("modules.evidence.indexing.reranker.rerank", fake_rerank)
    ...
    async def test_reranker_failure_keeps_original_order_and_returns_warning(
        ...
        monkeypatch.setattr(
            "modules.evidence.indexing.reranker.rerank",
            rerank_mock,
        )
        monkeypatch.setattr(
            "modules.evidence.indexing.retrieval._is_rerank_enabled",
            lambda _mode: True,
        )
    ...
    async def test_reranker_managed_step_uses_full_total_timeout(
        ...
        monkeypatch.setattr(
            "modules.evidence.indexing.reranker.run_managed_structured",
            managed,
        )
    ...
    async def test_default_circuit_breaker_provider_uses_novel_id(self) -> None:
        ...
        with patch(
            "modules.evidence.indexing.retrieval.get_circuit_breaker",
