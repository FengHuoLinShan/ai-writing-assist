...
from modules.evidence.compilation.schemas import EvidenceSearchResponse
...
async def test_smart_search_keeps_primary_range_aligned_and_aggregates_parent_scenes(
    ...
    monkeypatch.setattr("modules.evidence.indexing.facade.retrieve", fake_retrieve)
    monkeypatch.setattr(
        "modules.evidence.indexing.facade.get_index_freshness", fresh_index
    )
...
async def test_working_smart_search_reports_pending_index_without_old_chunks(
    ...
    from modules.evidence.indexing.facade import request_chapter_index
    from modules.writing.facade import create_draft_only
