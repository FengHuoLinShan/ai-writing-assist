...
class TestCheckEarlyReveal:
    ...
    async def test_reveal_plan_large_gap(
        ...
        # 35-10=25 > 20，应产生警告
        large_gap_warnings = [
            w for w in warnings if w.type == "early_reveal" and "间隔" in w.message
        ]
...
class TestCheckTimeline:
    ...
    async def test_duplicate_chapter_index(
        ...
        dup_warnings = [
            w for w in warnings if w.type == "timeline_conflict" and "重复" in w.message
        ]
    ...
    async def test_foreshadowing_seed_after_payoff(
        ...
        warnings = await service._check_timeline(None, "", payload)
        fs_warnings = [
            w for w in warnings if w.type == "timeline_conflict" and "埋设" in w.message
        ]
...
class TestCheckDuplicates:
    ...
    async def test_duplicate_names_in_candidates(
        ...
        dup_warnings = [
            w for w in warnings if w.type == "duplicate" and "重复名称" in w.message
        ]
        assert len(dup_warnings) >= 1
    ...
    async def test_unique_names_no_warning(
        ...
        warnings = await service._check_duplicates(None, "", payload)
        dup_warnings = [w for w in warnings if w.type == "duplicate"]
        assert len(dup_warnings) == 0
...
class TestReviewReportRepository:
    ...
    async def test_get_by_novel(
        ...
        nid = uuid.UUID(hex=novel_id)
        await repo.create(
            db_session, novel_id=nid, target_type="plot_structure", decision="pass"
        )
        await repo.create(
            db_session, novel_id=nid, target_type="world_structure", decision="reject"
        )
...
class TestRevisionInstructions:
    ...
    def test_schema_instructions(self, service: ReviewService) -> None:
        ...
        schema_instructions = [i for i in instructions if "Schema" in i]
