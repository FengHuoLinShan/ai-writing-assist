...
async def test_smart_resolution_does_not_reintroduce_blanket_adoption_queue(
    monkeypatch, review_enabled
):
    ...
    progress = DeepImportProgress(
        phase="done",
        review_resolution={"counts": {"optional": 1}} if review_enabled else {},
    )
    ...
    monkeypatch.setattr(
        orchestrator, "_restore_llm_execution_snapshot", AsyncMock(return_value={})
    )
    assemble = AsyncMock()
