"""Synthetic ten-chapter import regression coverage."""
...
FIRST_CHAPTER_TITLE = "第一章 雨夜来信"
LAST_CHAPTER_TITLE = "第十章 新的晨光"
...
class TestSyntheticFileParser:
    """Tracer Bullet: parser coverage without a machine-local novel file."""

    @pytest.fixture(scope="class")
    def file_bytes(self) -> bytes:
        return SYNTHETIC_FILE_PATH.read_bytes()

    def test_parser_splits_into_ten_chapters(self, file_bytes: bytes):
    ...
    def test_first_chapter_content_starts_with_story(self, file_bytes: bytes):
        ...
        assert "林舟" in content[:200], f"第一章开头不像正文: {content[:200]}"

    def test_book_metadata_excluded_from_chapters(self, file_bytes: bytes):
        """Synthetic fixture metadata must not leak into chapter content."""
        chapters = parse_txt(file_bytes)
        for i, ch in enumerate(chapters):
            assert "SYNTHETIC_METADATA" not in ch["content"], f"第 {i + 1} 章包含元信息"

    def test_chapter_word_count_minimum(self, file_bytes: bytes):
        """Each synthetic chapter must retain enough Chinese content to chunk."""
        chapters = parse_txt(file_bytes)
        for i, ch in enumerate(chapters):
...
class TestSyntheticFileImportService:
    """Cycle 2: ImportService completes the synthetic full import path."""

    @pytest_asyncio.fixture
    async def real_project(self, db_session: AsyncSession) -> str:
        """Create a Project record for the import path."""
        pid = uuid.uuid4()
        project = Project(
            id=pid,
            title="合成小说导入测试",
    ...
    @pytest_asyncio.fixture
    async def synthetic_file_bytes(self) -> bytes:
        return SYNTHETIC_FILE_PATH.read_bytes()
    ...
    async def test_import_creates_done_record(
        ...
        assert resp.file_name == "synthetic_ten_chapters.txt"
    ...
    async def test_imported_chapters_written_to_drafts(
        ...
        for i, draft in enumerate(drafts):
            ...
            assert draft.content and len(draft.content) > 50, f"第 {i + 1} 章草稿正文过短"
    ...
    async def test_import_record_persisted_correctly(
        ...
        assert record.file_size == len(synthetic_file_bytes)
