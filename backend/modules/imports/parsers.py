...
CHAPTER_PATTERNS = [
    re.compile(r"^(?:第[一二三四五六七八九十百千万零\d]+[章节回话]|序章|序言|前言|楔子|引子).*", re.MULTILINE),
...
def parse_epub(data: bytes) -> list[dict[str, str]]:
    ...
    try:
        ...
        for spine_item in book.spine:
            ...
            title = title_tag.get_text(strip=True) if title_tag else (item.get_name() or "无标题")
...
def parse_html(data: bytes) -> list[dict[str, str]]:
    ...
    chapter_headings = [h for h in heading_tags if _looks_like_chapter(h.get_text(strip=True))]
    ...
    if chapter_headings:
        ...
        for i, heading in enumerate(chapter_headings):
            ...
            while cursor and cursor is not next_heading:
                if cursor.name in ("p", "div", "blockquote", "h1", "h2", "h3", "h4", "h5", "h6", "section"):
...
def _looks_like_chapter(text: str) -> bool:
    """判断标题文本是否看起来像章节标题"""
    keywords = ["章", "节", "回", "话", "序", "卷", "楔子", "chapter", "ch", "prologue", "preface"]
