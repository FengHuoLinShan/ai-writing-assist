...
class TestPlotThreadService:
    ...
    async def test_get_active_contract(
        ...
        svc = PlotThreadService()
        await svc.create(db_session, sample_novel_id, PlotThreadCreate(
            name="活跃线", thread_type="main", start_chapter=1, status="canonical",
        ))
...
class TestOutlineArcService:
    ...
    async def test_get_by_chapter(
        ...
        svc = OutlineArcService()
        await svc.create(db_session, sample_novel_id, OutlineArcCreate(
            title="卷一", start_chapter=1, end_chapter=5,
        ))
        await svc.create(db_session, sample_novel_id, OutlineArcCreate(
            title="卷二", start_chapter=6, end_chapter=10,
        ))
