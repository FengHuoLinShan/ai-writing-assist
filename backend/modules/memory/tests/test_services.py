...
from modules.memory.schemas import EventType, ChapterPanorama
...
class TestApplyEvents:
    ...
    def test_entity_removed(self, memory_service: MemoryService) -> None:
        ...
        event = _event(event_type=EventType.entity_removed, entity_id=eid, snapshot_after={})
        result = memory_service._apply_events(state, [event])
    ...
    def test_relation_established(self, memory_service: MemoryService) -> None:
        ...
        event = _event(
            ...
            snapshot_after={"id": str(rid), "source_id": "a", "target_id": "b", "relation_type": "father_of"},
        )
        result = memory_service._apply_events(state, [event])
    ...
    def test_relation_ended(self, memory_service: MemoryService) -> None:
        ...
        state = {
            "entities": [],
            "relations": [{"id": rid, "source_id": "a", "target_id": "b", "relation_type": "friend_of"}],
            "character_locations": {},
            "character_knowledge": [],
        }
        event = _event(
    ...
    def test_knowledge_changed(self, memory_service: MemoryService) -> None:
        ...
        event = _event(
            ...
            snapshot_after={"id": "k1", "character_id": "c1", "target_type": "entity", "knowledge_level": "full"},
        )
        result = memory_service._apply_events(state, [event])
    ...
    def test_multiple_events_sequential(self, memory_service: MemoryService) -> None:
        ...
        events = [
            _event(event_type=EventType.entity_created, entity_id=eid,
                   snapshot_after={"id": str(eid), "name": "王五"}),
            _event(event_type=EventType.entity_updated, entity_id=eid,
                   snapshot_after={"summary": "已更新"}),
            _event(event_type=EventType.entity_moved, entity_id=eid,
...
class TestDiffStates:
    ...
    def test_entity_created_diff(self, memory_service: MemoryService) -> None:
        ...
        before = {"entities": [], "relations": [], "character_locations": {}, "character_knowledge": []}
        after = {"entities": [{"id": "e1", "entity_type": "character", "name": "新角色"}],
                 "relations": [], "character_locations": {}, "character_knowledge": []}
    ...
    def test_entity_updated_diff(self, memory_service: MemoryService) -> None:
        ...
        before = {"entities": [{"id": "e1", "name": "旧名", "summary": "", "entity_type": "character"}],
                  "relations": [], "character_locations": {}, "character_knowledge": []}
        after = {"entities": [{"id": "e1", "name": "新名", "summary": "", "entity_type": "character"}],
                 "relations": [], "character_locations": {}, "character_knowledge": []}
        events = memory_service._diff_states(before, after)
    ...
    def test_entity_removed_diff(self, memory_service: MemoryService) -> None:
        ...
        before = {"entities": [{"id": "e1", "entity_type": "character", "name": "待移除"}],
                  "relations": [], "character_locations": {}, "character_knowledge": []}
        after = {"entities": [], "relations": [], "character_locations": {}, "character_knowledge": []}
        events = memory_service._diff_states(before, after)
    ...
    def test_relation_established_diff(self, memory_service: MemoryService) -> None:
        ...
        before = {"entities": [], "relations": [], "character_locations": {}, "character_knowledge": []}
        after = {"entities": [],
                 "relations": [{"id": "r1", "source_id": "a", "target_id": "b", "relation_type": "friend_of"}],
                 "character_locations": {}, "character_knowledge": []}
        events = memory_service._diff_states(before, after)
    ...
    def test_character_location_changed_diff(self, memory_service: MemoryService) -> None:
        ...
        before = {"entities": [], "relations": [],
                  "character_locations": {"c1": {"location_id": "loc-a", "text_state": "在A"}},
                  "character_knowledge": []}
        after = {"entities": [], "relations": [],
                 "character_locations": {"c1": {"location_id": "loc-b", "text_state": "到了B"}},
                 "character_knowledge": []}
        events = memory_service._diff_states(before, after)
...
class TestRecordEvents:
    ...
    async def test_record_batch(
        self, memory_service: MemoryService, db_with_project: AsyncSession, sample_novel_id: uuid.UUID,
    ) -> None:
        """记录 3 条事件，验证数量和 sequence"""
        nid = str(sample_novel_id)
        events_data = [
            {"event_type": "entity_created", "entity_id": str(uuid.uuid4()), "entity_type": "character",
             "snapshot_after": {"name": "A"}},
            {"event_type": "entity_created", "entity_id": str(uuid.uuid4()), "entity_type": "character",
             "snapshot_after": {"name": "B"}},
            {"event_type": "entity_moved", "entity_id": str(uuid.uuid4()), "entity_type": "character",
    ...
    async def test_record_overwrites_existing(
        ...
        """重新记录同一章事件，旧事件被清除"""
        nid = str(sample_novel_id)
        old_events = [{"event_type": "entity_created", "entity_id": str(uuid.uuid4()),
                        "snapshot_after": {"name": "old"}}]
        await memory_service.record_events(db_with_project, nid, 3, old_events)

        new_events = [
            {"event_type": "entity_created", "entity_id": str(uuid.uuid4()), "snapshot_after": {"name": "new1"}},
            {"event_type": "entity_created", "entity_id": str(uuid.uuid4()), "snapshot_after": {"name": "new2"}},
            {"event_type": "entity_created", "entity_id": str(uuid.uuid4()), "snapshot_after": {"name": "new3"}},
        ]
...
class TestReplayState:
    ...
    async def _seed_events(
        self, memory_service: MemoryService, db: AsyncSession, nid: str, chapter_index: int, count: int,
    ) -> None:
        for i in range(count):
            await memory_service.record_events(db, nid, chapter_index, [{
                "event_type": "entity_created",
                "entity_id": str(uuid.uuid4()),
                "entity_type": "character",
                "snapshot_after": {"name": f"entity_c{chapter_index}_s{i}", "id": str(uuid.uuid4())},
            }])
    ...
    async def test_replay_empty(
        ...
        """无快照无事件 → 返回空状态"""
        result = await memory_service.replay_state(db_with_project, str(sample_novel_id), 1)
    ...
    async def test_replay_events_only(
        ...
        eid = uuid.uuid4()
        events_data = [
            {"event_type": "entity_created", "entity_id": str(eid), "entity_type": "character",
             "snapshot_after": {"id": str(eid), "name": "张三", "entity_type": "character"}},
        ]
...
class TestMarkStale:
    ...
    async def test_mark_single_snapshot(
        ...
        await repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, full_state=empty_state)
    ...
    async def test_mark_partial(
        ...
        repo = SnapshotRepository()
        await repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, full_state=empty_state)
        await repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=10, full_state=empty_state)
        await repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=15, full_state=empty_state)
