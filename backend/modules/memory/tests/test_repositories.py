...
class TestEventRepository:
    ...
    async def test_get_by_chapter(
        ...
        eid = uuid.uuid4()
        # Ch3: 3条
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=3, sequence=1,
                                event_type="entity_created", snapshot_after={"n": 1}, entity_id=eid)
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=3, sequence=2,
                                event_type="entity_updated", snapshot_after={"n": 2}, entity_id=eid)
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=3, sequence=3,
                                event_type="entity_moved", snapshot_after={"loc": "A"}, entity_id=eid)
        # Ch5: 2条
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, sequence=1,
                                event_type="entity_created", snapshot_after={"n": 3}, entity_id=uuid.uuid4())
    ...
    async def test_get_by_chapter_range(
        ...
        eid = uuid.uuid4()
        for ch in range(1, 6):
            await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=ch, sequence=1,
                                    event_type="entity_created", snapshot_after={"ch": ch}, entity_id=eid)

        result = await event_repo.get_by_chapter_range(db_with_project, sample_novel_id, 2, 4)
        assert len(result) == 3
    ...
    async def test_get_by_entity(
        ...
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=1, sequence=1,
                                event_type="entity_created", snapshot_after={"x": 1}, entity_id=target_eid)
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=3, sequence=1,
                                event_type="entity_moved", snapshot_after={"x": 2}, entity_id=target_eid)
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=2, sequence=1,
                                event_type="entity_created", snapshot_after={"x": 3}, entity_id=other_eid)

        items, total = await event_repo.get_by_entity(db_with_project, sample_novel_id, target_eid)
        assert len(items) == 2
        assert total == 2
    ...
    async def test_get_by_entity_pagination(
        ...
        eid = uuid.uuid4()
        for i in range(30):
            await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=i + 1, sequence=1,
                                    event_type="entity_updated", snapshot_after={"i": i}, entity_id=eid)

        items, total = await event_repo.get_by_entity(db_with_project, sample_novel_id, eid, skip=10, limit=5)
        assert len(items) == 5
        assert total == 30
    ...
    async def test_delete_by_chapter(
        ...
        for ch in [3, 3, 3, 5, 5]:
            await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=ch, sequence=1,
                                    event_type="entity_updated", snapshot_after={"ch": ch}, entity_id=eid)
    ...
    async def test_delete_from_chapter(
        ...
        all_remaining = await event_repo.get_by_chapter_range(db_with_project, sample_novel_id, 1, 5)
    ...
    async def test_get_max_sequence(
        ...
        for seq in range(1, 4):
            await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=1,
                                    sequence=seq, event_type="entity_updated",
                                    snapshot_after={"s": seq}, entity_id=eid)
    ...
    async def test_novel_isolation(
        ...
        eid2 = uuid.uuid4()
        await event_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=1, sequence=1,
                                event_type="entity_created", snapshot_after={"n": "A"}, entity_id=eid1)
        await event_repo.create(db_with_project, novel_id=nid2, chapter_index=1, sequence=1,
                                event_type="entity_created", snapshot_after={"n": "B"}, entity_id=eid2)
...
class TestSnapshotRepository:
    ...
    async def test_create(
        ...
        snap = await snapshot_repo.create(
            ...
            novel_id=sample_novel_id,
            chapter_index=5,
            full_state={"entities": [{"id": "x", "name": "test"}], "relations": [], "character_locations": {}, "character_knowledge": []},
            events_until=10,
        )
    ...
    async def test_get_nearest(
        ...
        empty_state = {"entities": [], "relations": [], "character_locations": {}, "character_knowledge": []}
        await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, full_state=empty_state)
        await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=10, full_state=empty_state)
    ...
    async def test_mark_stale_from(
        ...
        await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, full_state=empty_state)
        await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=10, full_state=empty_state)
        await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=15, full_state=empty_state)
    ...
    async def test_delete_stale(
        ...
        empty_state = {"entities": [], "relations": [], "character_locations": {}, "character_knowledge": []}
        s1 = await snapshot_repo.create(db_with_project, novel_id=sample_novel_id, chapter_index=5, full_state=empty_state)
        # 手动改为 stale（用 update 语句）
        from sqlalchemy import update
        from modules.memory.models import MemorySnapshot
        stmt = update(MemorySnapshot).where(MemorySnapshot.id == s1.id).values(status="stale")
