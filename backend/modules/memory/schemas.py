...
class CharacterLocationShift(BaseModel):
    ...
    movement_type: str = Field(
        ..., description="行为动词，如：步行潜入、御剑飞行、传送、撤退至"
    )
...
class ChapterStateExtraction(BaseModel):
    ...
    character_shifts: list[CharacterLocationShift] = Field(
        default_factory=list, description="角色位移列表"
    )
    faction_shifts: list[FactionControlShift] = Field(
        default_factory=list, description="势力割据变更列表"
    )
