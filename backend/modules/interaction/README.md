...
故事生成 attempt 的 `length/看海` 续写会换 task id；统一信封以
`InteractionGenerationAttempt.id` 作为 `run_id`，并在 task 私有 meta 与 attempt 私有
`agent_checkpoint_json` 双向窄同步。新 task 只恢复同一 attempt 的账本，不重置累计预算；既有
`AgentRunBudget` 仍保留用于 Agent 工具/web 子预算。每个合法 manual/看海续段只追加一次
同分段 `author_resume` 额度，不移动 deadline；旧 task 终态不覆盖已转交的新 task 快照。
旧 pending/awaiting-continue attempt 缺少信封时，以已有 attempt id 建立稳定 run 并标记
...
- 不把模型固定称为 DM，仅使用 ADR-0023 有界单 Agent 或 ADR-0027 注册协作；无任意自治工具。
...
审查读取同一预算内的完整生成消息（含尾部最新读者选择），不另作 24K 字符截断。创作可以
推进获准的局部行动和对话；既有规则、知识截止及隐藏事实的限制仍由审查与释放门禁执行。

`interaction-story-v8` 保留既有叙事方式，明确用户限定的持有物、数量和能力不可由职业常识补齐；
知识复审另列最新用户消息作为要求投影，与完整当前分支上下文同时核对。

## 多角色演绎（ADR-0027）

`generation_mode=ensemble` 是仅对已登录、绑定冻结作品资料旅程开放的实验选项，默认关闭。
Project 快照构造入口在计算 `profile_hash` 前冻结 v3/team_v1；业务模块不能自行改写已签名快照。
NPC 仅从当前场景提及且可见的人物中选择（最多三名），私有历史只读取选中祖先的 actor state。
无历史保持未知，离场人物不继续行动；不读取当前作者正史或未选兄弟分支。
...
已登录互动作品可选配对 CLI，并在每次 story attempt/续写根任务确认本机权限。
