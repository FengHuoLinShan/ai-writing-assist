...
- 世界生成中心的受管模型步骤总上限为 1800 秒。服务在 provider 调用前提交准备阶段
  checkpoint，确保等待期间没有数据库事务；结构化结果返回后在落库事务内重新校验项目仍
  active 并持有共享项目锁，同时以 page→draft 行锁重读并重验来源页面
  baseline，避免复用 provider 运行前的陈旧工作稿。项目已进入回收站时返回 404，来源
  发生变化时以 409 丢弃过时建议，不在长事务中等待或静默落库。多轮对话的决策编译、
