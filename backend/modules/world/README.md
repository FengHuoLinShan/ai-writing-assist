...
- 世界生成中心的受管模型步骤总上限为 1800 秒。服务在 provider 调用前提交准备阶段
  checkpoint，确保等待期间没有数据库事务；聊天或结构化结果返回后按同一准备流程只读复编，
  重新校验来源页面 baseline、所选章节/资产、显式 Scene/剧情线/人物/对象、世界观简介、
  Activation Profile 和实际提示资料。任一实际输入发生变化时以 409 丢弃过时结果，
  不在长事务中等待或静默落库，也不为这次复验建立第二条调用快照。多轮对话的决策编译、
