...
def upgrade() -> None:
    ...
    if "generation_prompt_templates" not in tables:
        op.create_table(
    ...
    template_indexes = {
        index["name"]
        for index in sa.inspect(op.get_bind()).get_indexes("generation_prompt_templates")
    }
    if "ix_generation_prompt_templates_novel_target_status_updated" not in (
        template_indexes
    ):
        op.create_index(
    ...
    if "generation_prompt_template_revisions" not in tables:
        op.create_table(
    ...
    if "ix_generation_prompt_template_revisions_novel_template" not in (revision_indexes):
        op.create_index(
