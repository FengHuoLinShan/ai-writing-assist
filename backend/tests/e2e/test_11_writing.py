...
class TestWritingMissingFlows:
    ...
    async def test_save_and_analyze(self, ctx):
        client, pid = ctx
        resp = await client.post("/api/writing/save-and-analyze", json={
            "novel_id": pid,
            "chapter_index": 1,
            "content": "测试正文",
        })
    ...
    async def test_update_draft_status(self, ctx):
        client, pid = ctx
        create_resp = await client.post("/api/writing/drafts", json={
            "novel_id": pid, "chapter_index": 5,
            "title": "待审批草稿", "content": "测试内容",
        })
