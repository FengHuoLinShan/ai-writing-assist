...
class TestTimelineCRUD:
    ...
    async def test_create_event(self, ctx):
        client, pid = ctx
        resp = await client.post(f"/api/novels/{pid}/timeline/events", json={
            "novel_id": pid, "title": "克莱恩穿越", "summary": "从灰雾之上穿越",
            "order_index": 1, "chapter_index": 1, "event_type": "plot",
        })
    ...
    async def test_list_events(self, ctx):
        client, pid = ctx
        for i in range(3):
            await client.post(f"/api/novels/{pid}/timeline/events", json={
                "novel_id": pid, "title": f"事件{i}", "summary": f"摘要{i}",
                "order_index": i + 1, "chapter_index": i + 1,
            })
    ...
    async def test_update_event(self, ctx):
        client, pid = ctx
        create = await client.post(f"/api/novels/{pid}/timeline/events", json={
            "novel_id": pid, "title": "原标题", "summary": "摘要",
            "order_index": 1, "chapter_index": 1,
        })
        eid = create.json()["id"]
        update = await client.put(f"/api/novels/{pid}/timeline/events/{eid}", json={
            "title": "新标题", "visibility": "reader_known",
        })
    ...
    async def test_delete_event(self, ctx):
        client, pid = ctx
        create = await client.post(f"/api/novels/{pid}/timeline/events", json={
            "novel_id": pid, "title": "待删除", "summary": "摘要",
            "order_index": 99, "chapter_index": 5,
        })
        eid = create.json()["id"]
