...
async def test_get_global_llm_defaults_missing_returns_200_null(  # noqa: E501
    ...
    r = await async_client.get("/api/account/settings/llm-defaults")
...
async def test_get_settings_does_not_require_xhr_header(async_client: AsyncClient):
    r = await async_client.get("/api/account/settings/author-preferences")
    assert r.status_code == 200
...
async def test_put_global_llm_defaults_round_trip(async_client: AsyncClient):
    ...
    r2 = await async_client.get("/api/account/settings/llm-defaults")
...
async def test_put_global_author_prefs_accepts_xhr_header(async_client: AsyncClient):
    r = await async_client.put(
        "/api/account/settings/author-preferences",
        headers=XHR_HEADERS,
...
async def test_project_author_prefs_missing_returns_empty_object(  # noqa: E501
    ...
    r = await async_client.get(f"/api/projects/{pid}/author-preferences")
    assert r.status_code == 200
    body = r.json()
...
async def test_project_author_prefs_field_reset_returns_400_for_unknown(  # noqa: E501
    ...
    r = await async_client.delete(
        f"/api/projects/{pid}/author-preferences/field/malicious_field",
        headers=XHR_HEADERS,
    )
...
async def test_projects_using_defaults_lists_inheriting(  # noqa: E501
    ...
    await async_client.put(
        f"/api/projects/{p1}/author-preferences",
        headers=XHR_HEADERS,
        json={
            "daily_goal": 1000,
            "editor_font": "serif",
    ...
    r = await async_client.get("/api/account/settings/projects-using-defaults")
    assert r.status_code == 200
...
async def test_refresh_endpoint(async_client: AsyncClient):
    r = await async_client.post("/api/account/settings/refresh", headers=XHR_HEADERS)
    assert r.status_code == 200
...
async def test_project_preferences_hide_recycled_project(
    ...
    get_response = await async_client.get(f"/api/projects/{pid}/author-preferences")
    put_response = await async_client.put(
        f"/api/projects/{pid}/author-preferences",
        headers=XHR_HEADERS,
        json={"daily_goal": 800},
    )
    delete_response = await async_client.delete(
        f"/api/projects/{pid}/author-preferences/field/daily_goal",
        headers=XHR_HEADERS,
    )
...
async def test_global_settings_endpoints_remain_project_guard_exempt(
    ...
    response = await async_client.get("/api/account/settings/llm-defaults")
