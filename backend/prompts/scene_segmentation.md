...
2. **Scene 与章节不是一对一**：一个 Scene 可横跨一至多章，同一章也可包含多个拥有独立主要叙事目标的 Scene。`scene_chunks` 记录物理映射。
3. **叙事标签判定**：
   - `hook` — 开篇钩子（黄金三章）
...
- **语义粒度**：以独立主要叙事目标、冲突或关键状态转变为边界，不使用字数或段落数阈值
- **主要 Scene 优先**：普通动作、过场和弱关联片段吸收到邻近主要 Scene；伏笔、揭示、目标或状态转变即使很短也不能丢失
...
2. 每个 Scene 是否真的具有独立主要叙事任务，普通过渡是否已吸收？
3. Overlap 章是否正确处理？
