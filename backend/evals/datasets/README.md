...
`baselines/rp-long-memory-v2.jsonl` is the current copyright-safe synthetic contract for
...
compaction. It adds oracle-only semantic fact rules; those rules are never rendered to
the model. `rp-long-memory-v1.jsonl` remains compile-only evidence for the superseded
exact-string protocol. The default command performs no database, network, key, or model I/O:
...
The model stage also writes an arm-blind candidate packet and a blank review template;
neither contains arm, case, or run labels. A cache-only replay re-scores raw probe output
with the current deterministic scorer and records both replay hits and the number of
records originally generated by the provider; it never opens a model client.
It also writes a rubric plus eight arm-free synthetic calibration candidates and a blank
calibration template. Reviewers must not open the arm map, model report, cache, dataset,
or sealed calibration reference while scoring. Every main reviewer must score every main
