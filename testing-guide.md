...
确定性用例覆盖预算竞争、来源/引文、成功回执复用、局部/全局失败、盲读顺序、情境保留、
方案选择与回归、同回合隔离、排演分叉及 RP held 正式提交。
PostgreSQL 用例 `tests/e2e/test_agent_teams_runtime.py` 与 `test_team_history.py` 检查
真实 worker 租约、取消、连接回收、20 节点选中祖先和不可变历史；只对专用可丢弃数据库
设置 `RUN_E2E_TESTS=1` 与 `E2E_DATABASE_URL`。受保护 Guimi 库不得作为这些夹具。
